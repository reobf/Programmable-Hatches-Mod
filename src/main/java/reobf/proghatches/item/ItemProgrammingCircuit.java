...
public class ItemProgrammingCircuit extends Item {
    ...
    public void addInformation(ItemStack p_77624_1_, EntityPlayer p_77624_2_, List p_77624_3_, boolean p_77624_4_) {
        ...
        String sid = Optional.ofNullable(p_77624_1_.stackTagCompound)
            .map(s -> s.getCompoundTag("targetCircuit"))
            .map(s -> s.getString("string_id"))
            .orElse(null);

        boolean iid = Optional.ofNullable(p_77624_1_.stackTagCompound)
            .map(s -> s.getCompoundTag("targetCircuit"))
            .filter(s -> s.hasKey("id"))
            .isPresent();
    ...
    public static int isNew(ItemStack is) {
        ...
        if (!is.getTagCompound()
            .hasKey("targetCircuit")) return 999;// ?
        boolean s = is.getTagCompound()
            .getCompoundTag("targetCircuit")
            .hasKey("string_id");
        boolean i = is.getTagCompound()
            .getCompoundTag("targetCircuit")
            .hasKey("id");
