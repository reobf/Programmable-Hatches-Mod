...
public class VoidOutputHatch extends MTEHatchOutput {
	...
	public void rebuildFilter() {
		...
		for (FluidStack is : filter) {
			if (is != null) {
				filterPredicate = filterPredicate.or(s -> {
	...
	public void addUIWidgets(Builder builder, UIBuildContext buildContext) {
		...
		for (int ii = 0; ii < 16; ii++) {
			...
			builder.widget(FluidSlotWidget.phantom(new MappingFluidTank(s -> {
				filter[i] = s == null ? null : s.copy();
				rebuildFilter();
			}, () -> filter[i]), false).setPos(3 + (i % 4) * 16, 3 + (i / 4) * 16));

			;
		}

		/*
		 * builder .widget( SlotGroup.ofItemHandler(handler,
		 * 4).startFromSlot(0).endAtSlot(15)
		 * .background(getGUITextureSet().getItemSlot()) .slotCreator(i -> new
		 * BaseSlot(handler, i, true) {
		 * 
		 * @Override public ItemStack getStack() { return isEnabled() ?
		 * super.getStack() : null; } }).build().setPos(7, 24));
		 */
	}
	...
	public VoidOutputHatch(int aID, String aName, String aNameRegional, int tier) {
		super(aID, aName, aNameRegional, tier,
				...
				reobf.proghatches.main.Config.get("VOH", ImmutableMap.of())
		...
		if (b && fx)
			MyMod.net.sendToAllAround(
					...
					new TargetPoint(this.getBaseMetaTileEntity().getWorld().provider.dimensionId,
							...
							this.getBaseMetaTileEntity().getZCoord(), 64));
	...
	public void onPreTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
		 ...
		 * this.getBaseMetaTileEntity(), new FluidStack(FluidRegistry.LAVA,
		 * 1000)
		 * ),this.getBaseMetaTileEntity().getWorld().provider.dimensionId); try{
		 * MyMod.net.sendToDimension(new VoidFXMessage(
		 * this.getBaseMetaTileEntity(), new
		 * FluidStack(FluidRegistry.getFluid("liquid_drillingfluid"), 1000)
		...
		if (remainticks > 0 && aBaseMetaTileEntity.getWorld().isRemote) {
			...
			/*
			 * for(long k=1;k<Integer.MAX_VALUE;k=k*100) if(remainticks>k*100){
			 * remainticks-=k; }
			 */
			...
			EntityDropParticleFX fx = new EntityDropParticleFX(Minecraft.getMinecraft().theWorld,
					...
					aBaseMetaTileEntity.getZCoord() + 0.5D + (fc.offsetZ) * 0.51f, f.getFluid());
	...
	public class EntityDropParticleFX extends EntityFX {
		...
		public void renderParticle(Tessellator tess, float timeStep, float rotationX, float rotationXZ, float rotationZ,
			...
			for (int i = 0; i < 5; i++) {
				renderParticle(tess, x, y, z, rotationX, rotationXZ, rotationZ, rotationYZ, rotationXY, minU, maxU,
						minV, maxV, scale);
			}
		}
		...
		private void renderParticle(Tessellator tess, double x, double y, double z, float rotationX, float rotationXZ,
				...
				float scale) {
			tess.addVertexWithUV((x - rotationX * scale - rotationYZ * scale), (y - rotationXZ * scale),
					(z - rotationZ * scale - rotationXY * scale), maxU, maxV);
			tess.addVertexWithUV((x - rotationX * scale + rotationYZ * scale), (y + rotationXZ * scale),
					(z - rotationZ * scale + rotationXY * scale), maxU, minV);
			tess.addVertexWithUV((x + rotationX * scale + rotationYZ * scale), (y + rotationXZ * scale),
					(z + rotationZ * scale + rotationXY * scale), minU, minV);
			tess.addVertexWithUV((x + rotationX * scale - rotationYZ * scale), (y - rotationXZ * scale),
					(z + rotationZ * scale - rotationXY * scale), minU, maxV);
		}
	...
	public void onScrewdriverRightClick(ForgeDirection side, EntityPlayer aPlayer, float aX, float aY, float aZ,
		...
		if (!getBaseMetaTileEntity().getCoverInfoAtSide(side).isGUIClickable())
			return;
		fx = !fx;
