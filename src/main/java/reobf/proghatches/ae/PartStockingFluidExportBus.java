...
public void writeToNBT(NBTTagCompound data) {
	if(inv[0]!=null){
		data.setTag("theslot",
		inv[0].writeToNBT(new NBTTagCompound()));}
	...
	public TickRateModulation tickingRequest(IGridNode node, int TicksSinceLastCall) {
		 ...
		 didSomething=false;
			 ...
			 if(iae==null)break;
			  ...
			  final IAEFluidStack itemsToAdd =  inv.extractItems(iae.copy().setStackSize(itemToSend), Actionable.SIMULATE, this.mySrc);
	 ...
	 private void pushItemIntoTarget(final  IFluidHandler d, final IEnergyGrid energy,
	        ...
	        if (canFit > 0) {
	            ...
	            if (itemsToAdd != null) {
	                ...
	                final long failed = itemsToAdd.getStackSize()-d.fill(side.getOpposite(),itemsToAdd.getFluidStack(),true);
	                if (failed >0) {
