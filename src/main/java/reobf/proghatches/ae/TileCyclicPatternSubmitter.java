...
public class TileCyclicPatternSubmitter extends TileEntity
    implements IGridProxyable, ICraftingRequester, ITileWithModularUI {
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        for (int i = 0; i < inv.length; ++i) if (compound.hasKey("inv" + i))
    ...
    public void updateEntity() {
        ...
        if (!getWorldObj().isRemote && (ticksSinceLoaded % 20 == 2)
            && on
            && (this.getProxy()
                .isActive()
                && this.getProxy()
    ...
    public CraftingCPUCluster getCpu(ICraftingLink cid) {
        try {
                ...
                if (ret instanceof CraftingCPUCluster) {
                    return (CraftingCPUCluster) ret;
                }
            } catch (Exception e) {
                broken = true;
                e.printStackTrace();
            ...
            if (found == null) {
                found = (CraftingCPUCluster) getProxy().getCrafting()
                    .getCpus()
                    .stream()
                    .filter(s -> {
    ...
    public CraftingCPUCluster getCpu(String cid) {
        try {
            return (CraftingCPUCluster) getProxy().getCrafting()
                .getCpus()
                .stream()
                .filter(s -> {
                    if (s instanceof CraftingCPUCluster) {
        ...
        protected void addUIWidgets(ModularWindow.Builder builder) {
            ...
            builder.widget(
                new DynamicTextWidget(
                    () -> new Text(StatCollector.translateToLocal("proghatches.submitter.state." + state)))
                        .setSynced(false)
                        .setPos(5, 3));
            ...
            builder.widget(new FakeSyncWidget.IntegerSyncer(() -> index, s -> {
                index = s;
                updateComparator();
            }));
            ...
            final IDrawable[] special = new IDrawable[] { GUITextureSet.DEFAULT.getItemSlot(),
                GTUITextures.OVERLAY_SLOT_PATTERN_ME, new ItemDrawable(new ItemStack(MyMod.progcircuit))
            ...
            builder.widget(
                new CycleButtonWidget().setToggle(() -> abortingMode, s -> abortingMode = s)
                    .setTextureGetter(s -> {
             ...
             */
            builder.widget(
                new CycleButtonWidget().setToggle(() -> asManyAsPossible, s -> asManyAsPossible = s)
                    .setTextureGetter(s -> {
                        if (s == 0) return GTUITextures.OVERLAY_BUTTON_CROSS;
            ...
            builder.widget(
                new CycleButtonWidget().setToggle(() -> skipIfFail, s -> skipIfFail = s)
                    .setTextureGetter(s -> {
                        if (s == 0) return GTUITextures.OVERLAY_BUTTON_CROSS;
