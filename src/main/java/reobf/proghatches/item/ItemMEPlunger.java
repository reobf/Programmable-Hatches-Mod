...
public class ItemMEPlunger extends DummySuper implements INetworkEncodable {
            ...
            if (mTileEntity instanceof IDualInputHatch/* BufferedDualInputHatch */) {
                ...
                for (IDualInputInventory inv : (Iterable<IDualInputInventory>) (Iterable) (() -> dual.inventories())) {
                    ...
                    for (ItemStack is : inv.getItemInputs()) {
                        ...
                        is.stackSize = Optional.fromNullable(left)
                            .transform(s -> s.getStackSize())
                            .or(0l)
                            .intValue();
                    }
                    for (FluidStack is : inv.getFluidInputs()) {
                        ...
                        IAEFluidStack left = storage.getFluidInventory()
                            .injectItems(
                                ...
                                Actionable.MODULATE,
                                new PlayerSource(aPlayer, (IActionHost) getWirelessGridHost(aStack)));
                        is.amount = Optional.fromNullable(left)
                            .transform(s -> s.getStackSize())
                            .or(0l)
                            .intValue();
                    }
    ...
    public boolean clearFluid(ItemStack aStack, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ,
        ...
        IStorageGrid fluid = getWirelessGrid(aStack).getGrid()
            .getCache(IStorageGrid.class);
        /*
         * IAEItemStack notadd =
                ...
                if (((IFluidHandler) aTileEntity).drain(tDirection, 1000, false) != null) {
                    if ((aPlayer.capabilities.isCreativeMode) || damage(aStack)) {
                        ...
                        IAEFluidStack notadd = fluid.getFluidInventory()
                            .injectItems(
                                AEFluidStack
                                    .create(((IFluidHandler) aTileEntity).drain(tDirection, Integer.MAX_VALUE, false)),
                                Actionable.SIMULATE,
                                new PlayerSource(aPlayer, (IActionHost) getWirelessGridHost(aStack)));
                        ...
                        fluid.getFluidInventory()
                            .injectItems(
                                AEFluidStack
                                    .create(((IFluidHandler) aTileEntity).drain(tDirection, Integer.MAX_VALUE, true)),
                                Actionable.MODULATE,
                                new PlayerSource(aPlayer, (IActionHost) getWirelessGridHost(aStack)));
                    ...
                    for (int j = tTileEntity.getSizeInventory(); i < j; i++) {
                        if (tTileEntity.isValidSlot(i)) {
                            if ((tTileEntity.getStackInSlot(i) != null)
                                ...
                                IStorageGrid item = getWirelessGrid(aStack).getGrid()
                                    .getCache(IStorageGrid.class);
                                IAEItemStack notadd = item.getItemInventory()
                                    .injectItems(
                                        AEItemStack.create(tTileEntity.getStackInSlot(i)),
                                        Actionable.SIMULATE,
                                        new PlayerSource(aPlayer, (IActionHost) getWirelessGridHost(aStack)));
                                if (notadd != null && notadd.getStackSize() > 0) {
                                    continue;
    ...
    @SideOnly(Side.CLIENT)
    public void addCheckedInformation(final ItemStack p_77624_1_, final EntityPlayer p_77624_2_,
        final List<String> p_77624_3_, final boolean p_77624_4_) {
        /*
         ...
         * @Override
         * @SideOnly(Side.CLIENT)
         * public void addInformation(ItemStack p_77624_1_, EntityPlayer p_77624_2_, List p_77624_3_, boolean
         * p_77624_4_) {
         */
        // if(p_77624_1_.getItemDamage()==1)p_77624_3_.add("");
        IntStream.range(0, Integer.valueOf(StatCollector.translateToLocal("item.proghatch_me_plunger.tooltips")))
            .forEach(s -> p_77624_3_.add(LangManager.translateToLocal("item.proghatch_me_plunger.tooltips" + "." + s)));
