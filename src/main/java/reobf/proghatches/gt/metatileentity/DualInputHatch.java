...
static int[] AZERO={0};
    ...
    /*
     * public DualInputHatch(int id, String name, String nameRegional, int tier, boolean mMultiFluid,boolean
     * extraCircuit,
     * String... optional) {
            ...
            (optional.length > 0 ? optional
                : reobf.proghatches.main.Config.get(
                    ...
                    ImmutableMap.of(
                        ...
                        "cap",
                        format.format(getInventoryFluidLimit(tier, mMultiFluid)),
                        "mMultiFluid",
                        mMultiFluid,
                        "slots",
    ...
    private CycleButtonWidget createButton(Supplier<Integer> getter, IntConsumer setter,
        ...
        return (CycleButtonWidget) new CycleButtonWidget().setLength(len)
            .setGetter(getter)
    ...
    private Widget createButtonForbidden(Supplier<Boolean> getter, Consumer<Boolean> setter, UITexture picture,
        ...
        }.setTextureGetter(s -> IDrawable.EMPTY)
    ...
    private static final SizedDrawable t0 = new SizedDrawable(
    ...
    private static final SizedDrawable t1 = new SizedDrawable(
        ...
        1,
        1);
    private static final SizedDrawable t2 = new SizedDrawable(
        AdaptableUITexture.of("appliedenergistics2", "guis/states", 16, 16, 0)
    ...
    public void addUIWidgets(Builder builder, UIBuildContext buildContext) {
        ...
        if (!allowSelectCircuit()) builder.widget(createButtonSharedItem());
        ...
        builder.widget(createButton(() -> !disableFilter, val -> {
            ...
            () -> mTooltipCache.getData("programmable_hatches.gt.filtermode"),
        ...
        builder.widget(createButtonForbidden(() -> true, val -> {
            ...
            () -> mTooltipCache.getData("programmable_hatches.gt.separate"),
            1).setPos(7 + 1 * 18, 62 - moveButtons() * 18));
        if (mMultiFluid == true && showFluidLimit()) builder.widget(createButton(() -> fluidLimit, val -> {
            fluidLimit = val;
            // updateSlots();
        }, s -> {
            if (s == 0) return t0;
...

        {
            ...
            for (int i = 0; i < mStoredFluid.length; i++) {
                ...
                scrollable.widget(
                    new FluidSlotWidget(mStoredFluid[i]).setBackground(ModularUITextures.FLUID_SLOT)
                        .setPos(position0));

            }

            builder.widget(
                scrollable.setSize(18 * fluidSlotsPerRow(), 18 * 4)
                    .setPos(position));
        } /*
    ...
    public Widget circuitSlot(IItemHandlerModifiable inventory, int slot) {
        ...
        List<String> tooltip = Arrays.asList(
            ...
            EnumChatFormatting.DARK_GRAY + (LangManager.translateToLocal("GT5U.machines.select_circuit.tooltip.3")));
    ...
    private ModularWindow createSharedItemWindow(UIBuildContext buildContext) {
        ...
        for (int i = 0; i < shared.itemMEUpgrades; i++) {
            final int fi = i;
            builder.widget(
                SlotWidget.phantom(new ItemStackHandler(shared.markedItems), i)
                    .addTooltips(
            ...
            builder.widget(
                new DrawableWidget().setDrawable(ModularUITextures.ARROW_RIGHT)
                    ...
                    .setPos(8 - 1 + 18 * 2 + 4, 8 - 1 + 18 * posoffset)
                    .setSize(18, 18));
                ...
                public void detectAndSendChanges(boolean init) {
                    ...
                    if (cd-- < 0) {
                        ...
                        if (net != null) {
                            ...
                            if (cahce != null) {
                                IAEItemStack aeis = cahce.getItemInventory()
                                    .getStorageList()
                ...
                public void detectAndSendChanges(boolean init) {
                    ...
                    if (cd-- < 0) {
                        ...
                        if (net != null) {
                            IStorageGrid cahce = net.g.getCache(IStorageGrid.class);
                            if (cahce != null) {
                                IAEFluidStack aeis = cahce.getFluidInventory()
                                    .getStorageList()
        ...
        builder.widget(
            new FakeSyncWidget.BooleanSyncer(() -> shared.broken, s -> shared.broken = s)
    ...
    public Optional<IDualInputInventory> getFirstNonEmptyInventory() {
        ...
        for (FluidStack f : theInv.getFluidInputs()) {
            if (f != null && f.amount > 0) empty = false;
            break;
        }
        for (ItemStack f : theInv.getItemInputs()) {
            if (f != null && f.stackSize > 0) empty = false;
            break;
        }
    ...
    static public ItemStack[] ensureIntMax(ItemStack[] in) {
        ...
        for (int i = 0; i < in.length; i++) {
            ...
            Iterator<TreeSet<Source>> itr = count.values()
                .iterator();
            TreeSet<Source> thiz;
            for (; itr.hasNext();) {
                thiz = itr.next();
    ...
    static VargsFunction<Object[], FluidStack[]> asFluidStack = (s) -> Arrays.stream(s)
    ...
    public FluidStack getFluid() {
        for (FluidTank tFluid : mStoredFluid) {
            if (tFluid != null && tFluid.getFluidAmount() > 0) return tFluid.getFluid();
        }
        return null;
    }

    public FluidStack getFluid(int aSlot) {
        if (aSlot < 0 || aSlot >= getMaxType()) return null;
    ...
    public int getFirstEmptySlot() {
        for (int i = 0; i < mStoredFluid.length; i++) {
            if (mStoredFluid[i].getFluidAmount() == 0) return i;
        }
        return -1;
    }
    ...
    public boolean hasFluid(FluidStack aFluid) {
        ...
        for (FluidTank tFluid : mStoredFluid) {
            if (aFluid.isFluidEqual(tFluid.getFluid())) return true;
        }
        return false;
    }
    ...
    public int getFluidSlot(FluidStack tFluid) {
        ...
        for (int i = 0; i < mStoredFluid.length; i++) {
            if (tFluid.equals(mStoredFluid[i].getFluid())) return i;
        }
        return -1;
    }
    ...
    public void addFluid(FluidStack aFluid, int aSlot) {
        ...
        if (aFluid.equals(mStoredFluid[aSlot].getFluid())) mStoredFluid[aSlot].fill(aFluid, true);
        else if (mStoredFluid[aSlot].getFluid() == null) mStoredFluid[aSlot].setFluid(aFluid.copy());
    }
    ...
    public void program() {
        if (shared.isDummy()) {
            ...
            for (int slot : (Iterable<Integer>) () -> IntStream.range(0, getSlots(mTier))
                .iterator()) {
    ...
    public ITexture[] getTexturesActive(ITexture aBaseTexture) {
        if (mMultiFluid) return new ITexture[] { aBaseTexture, TextureFactory.of(OVERLAY_INPUT_HATCH_2x2) };
    ...
    public boolean versionInsensitiveContainsInput(FluidStack aFluid) {
         ...
         * cache1.put(aFluid.getClass(), MethodHandles.lookup().findGetter(this.getClass(), "mRecipeMap",
         * recipeMapClass));
         * return versionInsensitiveContainsInput(aFluid);
         * } catch (Throwable e) {
    ...
    protected ModularWindow createInsertionWindow(UIBuildContext buildContext) {
        ...
        builder.widget(
            SlotGroup.ofItemHandler(inventoryHandler, len)
                .widgetCreator(SlotWidget::new)
                .startFromSlot(0)
                .endAtSlot(/* mInventory.length-2 */9999)
                .background(background)
                .build()
                .setPos(3, 3)
        ...
        builder.widget(new SyncedWidget() {
            ...
            public void detectAndSendChanges(boolean init) {
                ...
                for (int i = 0; i < inventoryHandler.getSlots(); i++) {
                    ...
                    Optional.ofNullable(inventoryHandler.getStackInSlot(i))
                        ...
                        .ifPresent(s -> {
                            ...
                            if (mInventory[fi] != null) {
                                ...
                                if (eq) {
                                    int canInject = Math
                                        .min(oldsize, getInventoryStackLimit() - mInventory[fi].stackSize);
            ...
            public void onDestroy() {
                ...
                inventoryHandler.getStacks()
                    .forEach(s -> {
                        if (s != null) {
                            if (buildContext.getPlayer().worldObj.isRemote) return;
                            EntityItem entityitem = buildContext.getPlayer()
                                .dropPlayerItemWithRandomChoice(s.copy(), false);
                            if (entityitem != null) {
                                entityitem.delayBeforeCanPickup = 0;
                                entityitem.func_145797_a(
        ...
        builder.widget(
            new DrawableWidget().setDrawable(new ItemDrawable(new ItemStack(Blocks.hopper)))
                .addTooltips(tt)
    ...
    public class OptioanlSharedContents {
        ...
        public boolean endRecipeProcessing(MTEMultiBlockBase controller) {
            ...
            for (int i = 0; i < markedItems.size(); i++) {
                ...
                if (sis != null) {
                    int consumed = sis.stackSize - Optional.ofNullable(cis)
                        .map(s -> s.stackSize)
                        .orElse(0);
                    ...
                    IAEItemStack whatweget = cahce.getItemInventory()
                        .extractItems(AEItemStack.create(extract), Actionable.MODULATE, new MachineSource(net.h));
                    int numweget = 0;
            ...
            for (int i = 0; i < markedFluid.size(); i++) {
                ...
                if (sis != null) {
                    int consumed = sis.amount - Optional.ofNullable(cis)
                        .map(s -> s.amount)
                        .orElse(0);
                    if (consumed > 0 && storageMissing) return false;
                    ...
                    IAEFluidStack whatweget = cahce.getFluidInventory()
                        .extractItems(AEFluidStack.create(extract), Actionable.MODULATE, new MachineSource(net.h));
                    int numweget = 0;
                    if (whatweget != null) numweget = (int) whatweget.getStackSize();
                    if (numweget != consumed) {
                        return false;
        ...
        public void startRecipeProcessing() {
            ...
            IStorageGrid cahce = (IStorageGrid) Optional.ofNullable(getNetwork())
                .map(s -> s.g)
                .map(s -> s.getCache(IStorageGrid.class))
                .orElse(null);
            ...
            for (int i = 0; i < markedItems.size(); i++) {
                ...
                if (is == null || cahce == null) {
                    ...
                    ItemStack ris = (Optional.ofNullable(
                        cahce.getItemInventory()
                            .getStorageList()
                            .findPrecise(toextract))
                        .map(IAEItemStack::getItemStack)
                        .orElse(null));
                    shadowItems.add(ris);
                    cachedItems.add(
                        Optional.ofNullable(ris)
                            .map(s -> s.copy())
                            .orElse(null));
                }
            }
        ...
        public void onDestroy() {
            ...
            if (circuitUpgrades > 0) te.getWorld()
                .spawnEntityInWorld(
                    new EntityItem(
                        te.getWorld(),
                        te.getXCoord(),
                        te.getYCoord(),
                        te.getZCoord(),
                        new ItemStack(MyMod.upgrades, circuitUpgrades, 0)));
            if (itemMEUpgrades > 0) te.getWorld()
                .spawnEntityInWorld(
                    new EntityItem(
                        te.getWorld(),
                        te.getXCoord(),
                        te.getYCoord(),
                        te.getZCoord(),
                        new ItemStack(MyMod.upgrades, itemMEUpgrades, 1)));
            if (fluidMEUpgrades > 0) te.getWorld()
                .spawnEntityInWorld(
                    new EntityItem(
                        te.getWorld(),
                        te.getXCoord(),
                        te.getYCoord(),
                        te.getZCoord(),
                        new ItemStack(MyMod.upgrades, fluidMEUpgrades, 2)));
        }
    ...
    public boolean onRightclick(IGregTechTileEntity aBaseMetaTileEntity, EntityPlayer aPlayer) {
        if (!aBaseMetaTileEntity.isClientSide()) if (!shared.isDummy()) MyMod.net.sendTo(
            new UpgradesMessage(
    ...
    public Net getNetwork() {
        ...
        final TileEntity te = self.getWorld()
            .getTileEntity(
        ...
        if (te instanceof IPartHost) {
            ...
            try {
                final Object part = ((IPartHost) te).getPart(
                    self.getFrontFacing()
                        .getOpposite());
                if (part instanceof IInterfaceHost) {

                    g = new Net(
                        ((IInterfaceHost) part).getGridNode(ForgeDirection.UP)
                            .getGrid(),
                        (IInterfaceHost) part
        ...
        if (g == null) {
            if (te instanceof IInterfaceHost) {
                ...
                if (n != null) g = new Net(n.getGrid(), ((IInterfaceHost) te));
        ...
        if (g == null) {
            ...
            if (optCover instanceof AECover.Data) {
                ...
                if (iface != null) {
                    g = new Net(
                        iface.getGridNode(ForgeDirection.UP)
                            .getGrid(),
                        iface);
