...
public class ProghatchesUtil {
	...
	public static Byte getSignalAt(UUID uuid, int frequency, CoverAdvancedRedstoneReceiverBase.GateMode mode,
		...
		switch (mode) {
		case AND: {
			return (byte) (signals.values().stream().map(signal -> signal > 0).reduce(true,
					(signalA, signalB) -> signalA && signalB) ? 15 : 0);
		}
		case NAND: {
			return (byte) (signals.values().stream().map(signal -> signal > 0).reduce(true,
					(signalA, signalB) -> signalA && signalB) ? 0 : 15);
		}
		case OR: {
			return (byte) (signals.values().stream().map(signal -> signal > 0).reduce(false,
					(signalA, signalB) -> signalA || signalB) ? 15 : 0);
		}
		case NOR: {
			return (byte) (signals.values().stream().map(signal -> signal > 0).reduce(false,
	...
	public static int moveFromSlotToSlotSafe(IInventory fromInv, IInventory toInv, int aGrabFrom, int aPutTo,
		...
		if (fromInv.getSizeInventory() <= aGrabFrom || 0 > aGrabFrom || toInv.getSizeInventory() <= aPutTo
	...
	public static void attachZeroSizedStackRemover(Builder builder, UIBuildContext buildContext) {
		builder.widget(new SyncedWidget() {
			...
			Consumer<Widget> ticker = ss -> {
				...
				Optional.ofNullable(buildContext.getPlayer().inventory.getItemStack()).filter(s -> s.stackSize <= 0)
	...
	public static void handleUse(Object player, Object tile, Object priorityHost,
			Object actionHost, Object stack, Object side) {
	...
	public static void handleUse(EntityPlayer player, TileEntity tile, IPriorityHost priorityHost,
		...
		try {
			...
			//e.printStackTrace();
			try {
				m=ProghatchesUtil.class.getDeclaredMethod("handleUse", Object.class, Object.class, Object.class, Object.class, Object.class, Object.class);
			} catch (Exception e1) {
				e1.printStackTrace();
			}
		}
    	...
    	try {
			m.invoke(null, player,  tile,  priorityHost,
		             actionHost,  stack,  side);
		} catch (Exception e) {
