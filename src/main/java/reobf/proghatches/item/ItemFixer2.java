...
public class ItemFixer2 extends Item {
    ...
    public boolean onItemUse(ItemStack p_77648_1_, EntityPlayer p_77648_2_, World w, int x, int y, int z,
        ...
        if (g != null) {
            ...
            System.out.println(
                "Player tried to use fixer to fix patterns, if this info spams in log and server gets laggy, blame 'em.");
            System.out.println(
                "player:" + p_77648_2_
                    .getDisplayName() + ",fix:" + cnt[0] + ",dim:" + p_77648_2_.getEntityWorld().provider.dimensionId);
    ...
    public IAEItemStack fixCircuit(IAEItemStack gs, Runnable succ) {
        ...
        gs = gs.copy();
            ...
            if (gs.getStackSize()>1) {
                
                gs = gs.copy().setStackSize(1);
                succ.run();
            }
