...
public class StockingDualInputHatchME extends MTEHatchInputBus
    ...
    public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTimer) {
        if (getBaseMetaTileEntity().isServerSide()) {
            program();
            // issue #327: when an external switch such as GT's machine controller cover is applied to
            // the hatch itself, flipping it from off back to on never tells the controller, so the
            // multiblock idles until the player opens its GUI and forces a check. Watch for the
            // false->true edge of isAllowedToWork() and wake the watching controllers ourselves.
            // This also covers the AE network losing and regaining power.
    ...
    /** See onPostTick, issue #327: last tick's isAllowedToWork(), used to detect the false->true edge. */
