...
,LangManager.translateToLocal("programmable_hatches.addedby")
    ...
    public List<ItemStack> filterTakable(TileEntity e) {
        ...
        if (e instanceof ISidedInventory) {
            ...
            for (int i : side.getAccessibleSlotsFromSide(dir.ordinal())) {
                if (side.canExtractItem(i, side.getStackInSlot(i), dir.ordinal())) slots.add(i);
            } ;
            // }
    ...
    public void setInventorySlotContents(int aIndex, ItemStack aStack) {
        ...
        List<ItemStack> arr = getTile().map(this::filterTakable)
            .orElseGet(ArrayList::new);

        if (aIndex >= 0 && aIndex - 1 < arr.size()) arr.set(aIndex - 1, aStack);
    }
    ...
    public ItemStack getStackInSlot(int aIndex) {
        ...
        if (i == aIndex) return mInventory[i];
