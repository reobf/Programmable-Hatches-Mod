...
public class IngredientDistributor extends MTEEnhancedMultiBlockBase<IngredientDistributor>
	...
	IStructureDefinition<IngredientDistributor> STRUCTURE_DEFINITION = StructureDefinition
			...
			.addShape("first",
					StructureUtility
							.transpose(new String[][] { { "m", "f", "c" }, { "m", "f", "c" }, { "m", "f", "c" } }))
			.addShape("second",
					StructureUtility
							.transpose(new String[][] { { "m", "f", "c" }, { "~", "c", "c" }, { "m", "c", "c" } }))
			.addShape("third",
					StructureUtility
							.transpose(new String[][] { { "m", "f", "c" }, { "m", "c", "c" }, { "◎", "c", "c" } }))
			.addShape("piece",
					StructureUtility
							.transpose(new String[][] { { " ", " ", " " }, { " ", "f", " " }, { "h", "h", "h" } }))
			.addShape("piece_survival", StructureUtility
					.transpose(new String[][] { { " ", " ", " " }, { " ", "f", " " }, { "h", "※", "h" } }))

			.addShape("last", StructureUtility
					.transpose(new String[][] { { " ", " ", " " }, { " ", "f", " " }, { " ", "f", " " } }))

			.addElement('c',

					ofBlock(GregTechAPI.sBlockCasings4, 12)

			).addElement('f', StructureUtility.ofChain(GTStructureUtility.ofFrame(Materials.Terbium), ofFrameAdder(Materials.Yttrium, s -> ((IngredientDistributor) s).onCheapFrameFound())

			)).addElement('h', buildHatchAdder(IngredientDistributor.class).atLeast(HatchElement.OutputBus.withAdder(IngredientDistributor::addBus).withCount(s -> s.hasBusThisLayer ? 1 : 0), HatchElement.OutputHatch.withAdder(IngredientDistributor::addHatch).withCount(s -> s.hasHatchThisLayer ? 1 : 0))
					// .shouldSkip((a,b)->a.hasBusThisLayer)
					.casingIndex(
							CASING_INDEX)
					.dot(1).buildAndChain(GregTechAPI.sBlockCasings4, 0))
			.addElement('※',
					buildHatchAdder(IngredientDistributor.class)
							...
							// .shouldSkip((a,b)->a.hasBusThisLayer)
							.casingIndex(CASING_INDEX).dot(1).buildAndChain(GregTechAPI.sBlockCasings4, 0))

			.addElement('m',
					buildHatchAdder(IngredientDistributor.class).atLeast(HatchElement.Maintenance, HatchElement.Energy)
							.casingIndex(CASING_INDEX).dot(3).buildAndChain(buildHatchAdder(IngredientDistributor.class)
									.atLeast(new IHatchElement<IngredientDistributor>() {
										...
										public IGTHatchAdder<? super IngredientDistributor> adder() {
											...
											return (s, w, u) -> {
												...
												if (s.port != null)
													return false;
												if (w.getMetaTileEntity() instanceof CommunicationPortHatch == false)
													return false;
			...
			.addElement('◎', buildHatchAdder(IngredientDistributor.class)
					...
					.atLeast(
							...
							, HatchElement.InputBus, HatchElement.InputHatch
	...
	public boolean checkMachine(IGregTechTileEntity aBaseMetaTileEntity, ItemStack aStack) {
		isLiteVersion = false;
		port = null;
		if (checkPiece("first", 1, 1, 0) && checkPiece("second", 0, 1, 0) && checkPiece("third", -1, 1, 0)) {
		} else {
			return false;
		}
		int index = -2;
	...
	private boolean moveToOutpusME(IDualInputInventory opt) {
		...
		for (int index = 0; index < i.length; ++index) {
			try {
				IAEItemStack notadded = ((MTEHatchOutputBusME) mOutputBusses.get(index)).getProxy().getStorage()
						.getItemInventory().injectItems(AEApi.instance().storage().createItemStack(i[index]),
								Actionable.MODULATE, getActionSourceFor(mOutputBusses.get(index)));
		...
		for (int index = 0; index < f.length; ++index) {
			try {
				IAEFluidStack notadded = ((MTEHatchOutputME) mOutputHatches.get(index)).getProxy().getStorage()
						.getFluidInventory().injectItems(AEApi.instance().storage().createFluidStack(f[index]),
								Actionable.MODULATE, getActionSourceFor(mOutputHatches.get(index)));
	...
	static class TransferCheckResult {
		...
		public String format() {
			if (isSuccess)
				return "";// StatCollector.translateToLocalFormatted("proghatch.ingbuf.success");
	...
	private TransferCheckResult checkMEBus(MTEHatchOutputBusME bus, ItemStack check, int index) {
		...
		try {
			...
			while (itr.hasNext()) {
				...
				if (next != null)
					if ((next).isSameType(check) == false && next.getStackSize() > 0) {
						if (check == null)
			...
			if (check != null) {
				...
				if (notadded != null && notadded.getStackSize() > 0)
					return TransferCheckResult.ofFail(
							"inject.failure." + (bus.getProxy().isPowered() && bus.getProxy().isActive()) + ".bus",
							index, check.copy(), notadded.copy());
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	private TransferCheckResult checkMEHatch(MTEHatchOutputME bus, FluidStack check, int index) {
		...
		try {
			...
			if (check != null) {
				IAEFluidStack notadded = bus.getProxy().getStorage().getFluidInventory()
						.injectItems(AEFluidStack.create(check), Actionable.SIMULATE, getActionSourceFor(bus));
				if (notadded != null && notadded.getStackSize() > 0)
					return TransferCheckResult.ofFail(
							"inject.failure." + (bus.getProxy().isPowered() && bus.getProxy().isActive()) + ".hatch",
							index, check.copy(), notadded.copy());
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	private boolean moveToOutpus(IDualInputInventory opt, boolean checkSpace) {
					...
					if (i[index] != null) {
						...
						try {
							...
							notadded = (bus).getProxy().getStorage().getItemInventory().injectItems(
									AEItemStack.create(i[index]), Actionable.SIMULATE, getActionSourceFor(bus));

							if (notadded != null && notadded.getStackSize() > 0) {
								TransferCheckResult ret = TransferCheckResult.ofFail("inject.failure."
										+ (bus.getProxy().isPowered() && bus.getProxy().isActive()) + ".bus", index,
										i[index].copy(), notadded.copy());
								if (!ret.isSuccess) {
									lastfail = ret;
									return false;
								}
					...
					if (f[index] != null) {
						...
						try {
							...
							notadded = (bus).getProxy().getStorage().getFluidInventory().injectItems(
									AEFluidStack.create(f[index]), Actionable.SIMULATE, getActionSourceFor(bus));

							if (notadded != null && notadded.getStackSize() > 0) {
								TransferCheckResult ret = TransferCheckResult.ofFail("inject.failure."
										+ (bus.getProxy().isPowered() && bus.getProxy().isActive()) + ".hatch", index,
										i[index].copy(), notadded.copy());
								if (!ret.isSuccess) {
									lastfail = ret;
									return false;
		...
		for (int index = 0; index < i.length; ++index) {
			if (mOutputBusses.get(index) instanceof MTEHatchOutputBusME) {
				...
				try {
					IAEItemStack notadd = ((MTEHatchOutputBusME) mOutputBusses.get(index)).getProxy().getStorage()
							.getItemInventory().injectItems(AEApi.instance().storage().createItemStack(i[index]),
									Actionable.MODULATE, getActionSourceFor(mOutputBusses.get(index)));
					if (notadd != null)
		...
		for (int index = 0; index < f.length; ++index) {
			if (mOutputHatches.get(index) instanceof MTEHatchOutputME) {
				...
				try {
					IAEFluidStack notadd = ((MTEHatchOutputME) mOutputHatches.get(index)).getProxy().getStorage()
							.getFluidInventory().injectItems(AEApi.instance().storage().createFluidStack(f[index]),
									Actionable.MODULATE, getActionSourceFor(mOutputHatches.get(index)));
					if (notadd != null)
	...
	public CheckRecipeResult checkProcessing() {
		...
		calculateOverclockedNessMultiInternal(120, 100, 1, inputVoltage, false);
	...
	public void addUIWidgets(com.gtnewhorizons.modularui.api.screen.ModularWindow.Builder builder,
		...
		builder.widget(createPowerSwitchButton(builder)).widget(createBlockingModeButton(builder))
				.widget(new FakeSyncWidget.BooleanSyncer(() -> blocking, val -> {
					blocking = val;
				}).setSynced(true, true)).widget(new FakeSyncWidget.BooleanSyncer(() -> lockRecipe, val -> {
					lockRecipe = val;
				}).setSynced(true, false)).widget(new FakeSyncWidget.BooleanSyncer(() -> port != null, val -> {
					clientport = val;
				}).setSynced(true, false))

				.widget(new FakeSyncWidget.IntegerSyncer(() -> count, val -> {
					count = val;
				}).setSynced(true, false)).widget(new FakeSyncWidget.IntegerSyncer(() -> cd, val -> {
					cd = val;
				}).setSynced(true, false)).widget(new FakeSyncWidget.IntegerSyncer(() -> cdmax, val -> {
					cdmax = val;
				}).setSynced(true, false)).widget(new FakeSyncWidget.BooleanSyncer(() -> yield, val -> {
					yield = val;
				}).setSynced(true, false))
				.widget(new FakeSyncWidget.IntegerSyncer(() -> blockingNotAvailableReason, val -> {
	...
	ButtonWidget createBlockingModeButton(IWidgetBuilder<?> builder) {
		...
		Widget button = new ButtonWidget().setOnClick((clickData, widget) -> {
			blocking = !blocking;
		}).setPlayClickSoundResource(() -> blocking ? SoundResource.GUI_BUTTON_UP.resourceLocation
		...
		IntStream.range(0, Integer.valueOf(StatCollector.translateToLocal("proghatches.ingredientdistr.blocking.desc")))
				.forEach(s -> button
						.addTooltip(LangManager.translateToLocal("proghatches.ingredientdistr.blocking.desc." + s)));
	...
	protected void drawTexts(DynamicPositionedColumn screenElements, SlotWidget inventorySlot) {
		...
		screenElements.widget(new TextWidget().setStringSupplier(() -> {
			if (yield)
				return StatCollector.translateToLocal("proghatch.ingbuf.yield");
			if (count > 0)
				return StatCollector.translateToLocal("proghatch.ingbuf.acquring");
			if (lockRecipe)
				return StatCollector.translateToLocal("proghatch.ingbuf.locked");

			return StatCollector.translateToLocal("proghatch.ingbuf.idle");
		}

		).setDefaultColor(COLOR_TEXT_WHITE.get()).setEnabled(widget -> clientport));
		...
		screenElements.widget(new TextWidget().setStringSupplier(() -> StatCollector.translateToLocal(

				"proghatch.ingbuf.noblocking.reason." + blockingNotAvailableReason)

		).setDefaultColor(COLOR_TEXT_RED.get()).setEnabled(widget -> blockingNotAvailableReason > 0));
