...
public class DualInputHatchInventoryMappingSlave<T extends MetaTileEntity & IDualInputHatch & IMetaTileEntity>
    ...
    public ITexture[] getTexture(IGregTechTileEntity baseMetaTileEntity, ForgeDirection side, ForgeDirection facing,
        ...
        if (side != facing)
            ...
            TextureFactory.of(MyMod.iohub, BlockIOHub.magicNO_inv_me_slave), TextureFactory.of(OVERLAY_PIPE_IN) };
    ...
    public ForgeDirection getMasterFront() {
        if (master == null) return this.getBaseMetaTileEntity()
            .getFrontFacing();// throw new RuntimeExcpetion()?
    ...
    public boolean onWireCutterRightClick(ForgeDirection side, ForgeDirection wrenchingSide, EntityPlayer aPlayer,
        ...
        aPlayer
            .addChatComponentMessage(new ChatComponentTranslation("GT5U.hatch.additionalConnection." + allowAllSides));
        return true;
    }
    ...
    public boolean acceptsPlansCM() {
        if ((master = getMaster()) != null) {
            if (master instanceof ICraftingV2) return ((ICraftingV2) master).acceptsPlansCM();
        } ;
        return false;
    }
    ...
    public boolean enableCM() {
        if ((master = getMaster()) != null) {
            if (master instanceof ICraftingV2) return ((ICraftingV2) master).enableCM();
        } ;
        return false;
    }
}
