...
public class MAConduit extends AbstractConduit implements ICraftingMachineConduit{
	...
	
	{
		...
		public void addConduit(ICraftingMachineConduit con) {
			...
			if(((MAConduit) con).containsExternalConnection(dir)&&((MAConduit) con).getConnectionMode(dir)!=ConnectionMode.DISABLED)
	...
	public boolean setNetwork(AbstractConduitNetwork<?, ?> network) {
		 ...
		 if(updateLater&&net!=null){net.updateConn(this);updateLater=false;}
		 return true;
	}
	...
	private TileEntity getTarget(TileEntity thiz,ForgeDirection side) {
        ...
        final TileEntity te = thiz.getWorldObj().getTileEntity(
        		thiz.xCoord +side.offsetX,
        		thiz.yCoord + side.offsetY,
        		thiz.zCoord + side.offsetZ);
	...
	static{try {
	 ...
	 @Override
	    public boolean canConnectToExternal(ForgeDirection direction, boolean ignoreConnectionMode) {
	        ...
	        a:if (te instanceof IInterfaceHost) {
	        	if(ignoreConnectionMode==false&&!getConnectionMode(direction).acceptsInput()){break a;}
		           return true;
		        }
	 ...
	 public static class ConduitRenderer extends DefaultConduitRenderer {
			...
			try {Field f=null;
				...
				f.setAccessible(true);ICONS= (Map<String, IIcon>) f.get(null);
			} catch (Exception e) {
				e.printStackTrace();
			}
		        ...
		        for (ForgeDirection dir : conduit.getExternalConnections()) {
		            ...
		            if(getOffset==null)
		        		for(Method m:((Object)te).getClass().getMethods()){
		        			if(m.getName().equals("getOffset")){
		            ...
		            if (render && !rb.hasOverrideBlockTexture()) {
		                ...
		                ConnectionModeGeometry.renderModeConnector(dir, offset,ICONS.get(ItemConduit.ICON_KEY_IN_OUT_BG), true);
