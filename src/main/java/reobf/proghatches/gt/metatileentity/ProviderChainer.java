...
@Override
	...
	if(!ICircuitProvider.super.checkLoop(blacklist))return false;
		
	return !(forEachMachines(s->test[0]=test[0]||(!s.checkLoop(blacklist)),true)&&
				test[0]);
...
public Collection<ItemStack> getCircuit() {
	if(this.isActive()&&this.isPowered()){}else{
...
public boolean patternDirty(){
	...
	boolean[] test=new boolean[1];
	if(forEachMachines(s->{test[0]=test[0]||s.patternDirty();s.clearDirty();},true)&&test[0]){
