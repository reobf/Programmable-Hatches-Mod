...
public class TileWirelessPeripheralStation extends TileEntity implements li.cil.oc.api.network.Environment {
	...
	public NBTTagCompound getNBTData(EntityPlayerMP player, TileEntity te, NBTTagCompound tag, World world, int x, int y,
		...
		Optional.ofNullable(
		WirelessPeripheralManager.cards.get(
				((TileWirelessPeripheralStation)world.getTileEntity(x, y, z)).thisUUID
				))
		.ifPresent(
		s->{
			Optional.ofNullable(WirelessPeripheralManager.stations.get(
					((TileWirelessPeripheralStation)world.getTileEntity(x, y, z)).thisUUID
					))
			.ifPresent(w->{
...
public void addInformation(ItemStack p_77624_1_, EntityPlayer p_77624_2_, List p_77624_3_, boolean p_77624_4_) {
	...
	public static class Block extends net.minecraft.block.BlockContainer {
		...
		public boolean onBlockActivated(World worldIn, int x, int y, int z, EntityPlayer player, int side, float subX,
			...
			if ((is = player.getHeldItem()) != null&&is.getItem() instanceof ItemWirelessPeripheralCard) {
				...
				is.getTagCompound().setString("remoteUUID",
						tile.thisUUID.toString());
	...
	public void updateEntity() {
		...
		if(this.worldObj.isRemote){
			thisUUID=new UUID(0,0);//set to 0 on client to prevent Waila from showing random UUID
			
		}else{
			
			if(oneComputer^this.worldObj.getBlockMetadata(xCoord, yCoord, zCoord)==0)
			this.worldObj.setBlockMetadataWithNotify(xCoord, yCoord, zCoord, 1-this.worldObj.getBlockMetadata(xCoord, yCoord, zCoord), 2);
			
			if(!oneComputer){return;}
			if(node ==null)return;
			...
			node.network().nodes().forEach(s->{
				if(s.host() instanceof li.cil.oc.server.machine.Machine){
					...
					//if(mch.host() instanceof Computer)//won't compile... projectred interfaces missing
			...
			if(remoteID==null){inrange=true;}
			if(remoteID!=null){
				ItemWirelessPeripheralCard.Env me=(Env) remoteID.host();
				DoubleUnaryOperator op=s->s*s;
				double x=me.host.xPosition()
				      ,y=me.host.yPosition()
				      ,z=me.host.zPosition();
				double  x1=this.xCoord+0.5
					,y1=this.yCoord+0.5
					,z1=this.zCoord+0.5;
				double epsilon=0.01;
				 inrange=range*range+epsilon>=
						 op.applyAsDouble(x-x1)+ op.applyAsDouble(y-y1)+ op.applyAsDouble(z-z1)
						 &&me.host.world()==this.worldObj
						 ;
