...
public class AIFix extends EntityAIBase {
    ...
    public AIFix(Object mixinGolem) {
        ...
        if (this.theWorld instanceof WorldServer) {
            this.player = FakePlayerFactory
                .get((WorldServer) this.theWorld, new GameProfile((UUID) null, "FakeThaumcraftGolem"));
        }
    ...
    public boolean shouldExecute() {
        if (working == false && this.delay <= 0
            && this.theGolem.ticksExisted % Config.golemDelay <= 0
            && this.theGolem.getNavigator()
                .noPath()) {
    ...
    public void startExecuting() {
        ...
        this.theGolem.getNavigator()
            .tryMoveToXYZ(
                ...
                (double) this.yy + 0.5D,
                (double) this.zz + 0.5D,
                (double) this.theGolem.getAIMoveSpeed());
    }
    ...
    public boolean continueExecuting() {
        boolean b =
             ...
             */
            this.count-- > 0 && (this.delay > 0 || needFix(hostx, hosty, hostz)
                || !this.theGolem.getNavigator()
                    .noPath());
    ...
    public void updateTask() {
        double dist = this.theGolem
            .getDistanceSq((double) this.xx + 0.5D, (double) this.yy + 0.5D, (double) this.zz + 0.5D);
        this.theGolem.getLookHelper()
            .setLookPosition((double) this.xx + 0.5D, (double) this.yy + 0.5D, (double) this.zz + 0.5D, 30.0F, 30.0F);
        if (dist <= 4.0D) {
