...
public class WirelessControlCover extends CoverBehaviorBase<WirelessControlCover.Data>
	...
	public class Data implements ISerializableObject {
		...
		public void writeToByteBuf(ByteBuf aBuf) {
			aBuf.writeBoolean(privateFreq).writeBoolean(invert).writeBoolean(safe).writeBoolean(crashed).writeInt(freq)
					.writeBoolean(useMachineOwnerUUID).writeInt(gateMode).writeBoolean(user != null);
			;
			if (user != null)
				aBuf.writeLong(user.getMostSignificantBits()).writeLong(user.getLeastSignificantBits())
	...
	private class WirelessCCUIFactory extends UIFactory {
		...
		protected void addUIWidgets(ModularWindow.Builder builder) {
			...
			builder.widget(((CycleButtonWidget) new CoverCycleButtonWidget().setSynced(false, true))
					.setGetter(() -> getCoverData().crashed ? 1 : 0).setSetter(s -> getCoverData().crashed = s == 1)
					.setLength(2).setTextureGetter(i -> {
						if (i == 1)
							return OFF;
						return ON;
			...
			builder.widget(((CycleButtonWidget) new CoverCycleButtonWidget().setSynced(false, true))
					...
					.setSetter(s -> getCoverData().useMachineOwnerUUID = s == 1).setLength(2).setTextureGetter(i -> {
						if (i == 1)
							return MACHINE;
						return COVER;
					})
			...
			builder.widget(((CycleButtonWidget) new CoverCycleButtonWidget().setSynced(false, true))
					.setGetter(() -> getCoverData().gateMode).setSetter(s -> getCoverData().gateMode = s)
			...
			}).setPos(startX, startY + spaceY * 3).setSize(spaceX * 2 + 5, 12)
			...
			builder.widget(Optional.of(new TextFieldWidget()).filter(s -> {
				...
				return getCoverData().freq + "";
			}).setSetter(s -> {
				try {
					getCoverData().user = getUIBuildContext().getPlayer().getUniqueID();
	...
	public static final UITexture OFF = UITexture.fullImage(GregTech.ID,
			"blocks/iconsets/OVERLAY_FRONT_IMPLOSION_COMPRESSOR.png");
	public static final UITexture ON = UITexture.fullImage(GregTech.ID,
