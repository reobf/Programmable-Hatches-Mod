...
public class EUUtil {
    ...
    public static void register() {
        AEApi.instance()
            .registries()
            .gridCache()
            .registerGridCache(IEUManager.class, EUManager.class);
        AEApi.instance()
            .registries()
            .gridCache()
            .registerGridCache(INodeAdditionCallback.class, NodeAdditionCallback.class);

        // AEApi.instance().registries().gridCache().registerGridCache(IChunkTrackingGridCahce.class,
        // ChunkTrackingGridCahce.class);
    ...
    public static boolean check(ISegmentedInventory inv) {
        ...
        for (int i = 0; i < sub.getSizeInventory(); i++) {
            if (Optional.ofNullable(sub.getStackInSlot(i))
    ...
    public static IGuiProvidingPart select(EntityPlayer player, int x, int y, int z) {
        ...
        // tag.removeTag(key);
        TileEntity tt = player.getEntityWorld()
            .getTileEntity(x, y, z);
    ...
    public static final UIInfo<?, ?> PART_MODULAR_UI = UIBuilder.of()
        .container((player, world, x, y, z) -> {
            return Optional.ofNullable(select(player, x, y, z))
                .map(part -> {
                    ...
                    return new ModularUIContainer(new ModularUIContext(buildContext, () -> {
                        ...
                        if (part instanceof IInventory) ((IInventory) part).markDirty();
                    }), window, null);
