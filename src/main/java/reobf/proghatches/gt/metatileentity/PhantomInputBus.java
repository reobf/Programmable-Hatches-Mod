...
import com.rwtema.extrautils.worldgen.Underdark.WorldGenCastle.Vec2;
	...
	public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTimer) {
	   ...
	   @Override
	    public void addUIWidgets(Builder builder, UIBuildContext buildContext) {
	        final Scrollable scrollable = (Scrollable) new Scrollable().setVerticalScroll().setSize(6, 40);
	                            ...
	                            .setGTTooltip(
	                                () -> new TooltipData(
	                                    ...
	                                    Arrays.asList(
	                                        LangManager.translateToLocal("programmable_hatches.gt.marking.slot.0"),
	                                        LangManager.translateToLocal("programmable_hatches.gt.marking.slot.1"))))
	                            .setSize(18, 18).setPos(column * 18, row * 18)
...
public void onLeftclick(IGregTechTileEntity aBaseMetaTileEntity, EntityPlayer aPlayer) {
    ...
    dataStick.getTagCompound().setString("type", "PhantomInput");
    dataStick.setStackDisplayName(
            "Phantom Input Bus Link");
    }
    if(dataStick.getTagCompound().hasKey("phantom_pos")==false){
    	dataStick.getTagCompound().setTag("phantom_pos",new NBTTagIntArray(new int[0]));
    };
    ...
    NBTTagIntArray tag = (NBTTagIntArray) dataStick.getTagCompound().getTag("phantom_pos");
    
    
    List<Vector4i> a=p(tag);Vector4i v;
    if(a.size()>=1024){
    	 aPlayer.addChatMessage(new  ChatComponentTranslation("proghatch.phantombus.chat.link.exceed"));
    }else
    
    if(!a.contains(v=new Vector4i(
    		this.getBaseMetaTileEntity().getXCoord(),this.getBaseMetaTileEntity().getYCoord(),this.getBaseMetaTileEntity().getZCoord()
    		,
    		this.getBaseMetaTileEntity().getWorld().provider.dimensionId
    		))){
    	a.add(v);aPlayer.addChatMessage(new ChatComponentTranslation("proghatch.phantombus.chat.link.success",a.size()));
    	
    }else{
    	 aPlayer.addChatMessage(new ChatComponentTranslation("proghatch.phantombus.chat.link.noop",a.size()));
    	
    }
    dataStick.getTagCompound().setTag("phantom_pos",p(a));
 }
...
public boolean onRightclick(IGregTechTileEntity aBaseMetaTileEntity, EntityPlayer aPlayer) {
	...
	ds:{
	  if (!(aPlayer instanceof EntityPlayerMP)) break ds;
	    ...
	    if(dataStick.getTagCompound().getString("type").equals("PhantomInput")==false){
	    	break ds;
	    };
	    ...
	    a.removeIf((aw)->{
		    ...
		    if(w==null){
		    	aPlayer.addChatMessage(new ChatComponentTranslation("proghatch.phantombus.chat.world.missing",aw.w));
		    	return false;}
		    if(w.blockExists(aw.x, aw.y, aw.z)==false){
		    	aPlayer.addChatMessage(new ChatComponentTranslation("proghatch.phantombus.chat.chunk.missing",aw.w,aw.x,aw.y,aw.z));
		    	  return false;};
		    
		    
		    TileEntity te = w.getTileEntity(aw.x, aw.y, aw.z);
		    if(te==null){
		    	aPlayer.addChatMessage(new ChatComponentTranslation("proghatch.phantombus.chat.te.missing",aw.w,aw.x,aw.y,aw.z));
			    
		     return true;
		    }
		    if(!(te instanceof IGregTechTileEntity)){
		    	aPlayer.addChatMessage(new ChatComponentTranslation("proghatch.phantombus.chat.te.incorrect",aw.w,aw.x,aw.y,aw.z));
			   return true;
		    }
