...
public class TileOrbSwitcher extends TileEntity implements IActionHost, IGridProxyable, ITileWithModularUI {
    ...
    public void pretick() {
        ...
        if ((!isDead) && (!isInvalid())) {
            if (this.worldObj.getTileEntity(xCoord, yCoord, zCoord) == this) {
                ...
                ForgeDirection dir = facing;// ForgeDirection.VALID_DIRECTIONS[ this.worldObj.getBlockMetadata(xCoord,
                                            // yCoord, zCoord)];
                select = null;
                ...
                try {
                    ...
                    IMEMonitor<IAEItemStack> stockinginv = stocking.getStorage()
                        .getItemInventory();
                    IMEMonitor<IAEItemStack> orbinv = orb.getStorage()
                        .getItemInventory();

                    boolean foundTarget = false;
                    for (IAEItemStack i : orb.getStorage()
                        .getItemInventory()
                        .getStorageList()) {
                        if (same(i, select)) {
                            foundTarget = true;
                            if (select.stackSize < i.getStackSize()) {
                                IAEItemStack get = orbinv.extractItems(
                                    i.copy()
                                        .setStackSize(-select.stackSize + i.getStackSize()),
                                    Actionable.MODULATE,
                                    source);
                                ...
                                IAEItemStack get = stockinginv.extractItems(
                                    i.copy()
                                        .setStackSize(select.stackSize - i.getStackSize()),
                                    Actionable.MODULATE,
                                    source);
                    ...
                    if (foundTarget == false && select != null) {
                        ...
                        IAEItemStack get = stockinginv
                            .extractItems(sel.setStackSize(select.stackSize), Actionable.MODULATE, source);
                        if (get == null) {
                            if (nbt == false || meta == false) {
                                // Collection<IAEItemStack> fuzz = stockinginv.getStorageList().findFuzzy(sel,
                                ...
                                for (IAEItemStack item : stockinginv.getStorageList()) {
                                    if (same(item, select)) {// find fuzzy might return some oredict items... check it!
                                        ...
                                        get = stockinginv.extractItems(
                                            item.copy()
                                                .setStackSize(select.stackSize),
                                            Actionable.MODULATE,
                                            source);
                                        if (get != null) break;
                                    }
    ...
    public boolean same(IAEItemStack a, ItemStack b) {
        ...
        if (meta) if (a.getItemDamage() != b.getItemDamage()) {
            return false;
        }
        if (nbt) if (Objects.equal(a.getTagCompound(), b.getTagCompound())) {
            return false;
        }
        return true;
    }
    ...
    public ModularWindow createWindow(UIBuildContext buildContext) {
        ...
        builder.widget(
            new NumericWidget().setSetter(val -> amount = (int) val)
                .setGetter(() -> amount)
                .setBounds(1, Integer.MAX_VALUE - 1)
