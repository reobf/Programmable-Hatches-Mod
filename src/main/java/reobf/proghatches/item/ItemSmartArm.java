...
public class ItemSmartArm extends Item {
	...
	public void addInformation(ItemStack p_77624_1_, EntityPlayer p_77624_2_, List p_77624_3_, boolean p_77624_4_) {
		...
		for (int i = 0; i < size; i++)
			p_77624_3_.add(LangManager.translateToLocalFormatted("item.proghatch.smartarm.name.tooltip." + i, sec, am));
	...
	public ItemStack onItemRightClick(ItemStack itemStackIn, World worldIn, EntityPlayer player) {
		...
		if (worldIn.isRemote == true && player.isSneaking()) {
			int size = Integer.valueOf(LangManager.translateToLocalFormatted("programmable_hatches.cover.smart.desc"));
			for (int i = 0; i < size; i++)
				player.addChatMessage(new ChatComponentText(
						LangManager.translateToLocal("programmable_hatches.cover.smart.desc." + i)));
		}
