...
public abstract class RequestTunnel implements ICraftingMachine,ICraftingRequester
		 ...
		 try{
			...
			if(last==null){
				if(job==null){
					if(req!=null){
						if(cd--<=0){
					job = getProxy().getCrafting().beginCraftingJob(getWorldObj(), getProxy().getGrid(), new MachineSource(this), req, null);
					cd=40;
						}}
					}
