...
public class SuperChestME extends MTEHatch implements ICellContainer, IGridProxyable, IPriorityHost, IStoageCellUpdate {
							...
							if (last.getStackSize() == mInventory[0].stackSize) {
								...
								this.getProxy().getStorage().postAlterationOfStoredItems(StorageChannel.ITEMS,
										ImmutableList.of(last.copy()
												.setStackSize(mInventory[0].stackSize - last.getStackSize())),
	...
	public ITexture[] getTexturesActive(ITexture aBaseTexture) {
		...
		return new ITexture[] { aBaseTexture, new IIconTexture(ExtraBlockTextures.MEChest.getIcon(), 0xD7BBEC),
				new IIconTexture(ExtraBlockTextures.BlockMEChestItems_Light.getIcon(), 0xffffff)
	...
	public boolean allowPutStack(IGregTechTileEntity aBaseMetaTileEntity, int aIndex, ForgeDirection side,
		...
		if(mInventory[0]!=null&&
				 aStack!=null&&(!Platform.isSameItem(mInventory[0], aStack))){
				return false;
			}
		return aIndex != 0;
	}
	...
	public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
		...
		if (!aBaseMetaTileEntity.getWorld().isRemote) {
			...
			if (mInventory[0] != null && voidOverflow) {
				...
				if (mInventory[0] != null)
					mInventory[0].stackSize = Math.min(cap(), mInventory[0].stackSize);
			}
		...
		for (int i = 1; i < mInventory.length; i++) {
			...
			if (is == null)
				continue;
			if (mInventory[0] != null) {
				if (mInventory[0].getItem() != is.getItem() || mInventory[0].getItemDamage() != is.getItemDamage() ||(!ItemStack.areItemStackTagsEqual(mInventory[0], is)))
{
	...
	public void addUIWidgets(Builder builder, UIBuildContext buildContext) {
		...
		IntStream.range(0, Integer.valueOf(StatCollector.translateToLocal("programmable_hatches.gt.mechest.tooltip")))
				.forEach(s -> w
						.addTooltip(LangManager.translateToLocal("programmable_hatches.gt.mechest.tooltip." + +s)));
		...
		builder.widget(new TextFieldWidget().setPattern(BaseTextFieldWidget.WHOLE_NUMS).setGetter(() -> piority + "")
				.setSetter(s -> {
					try {
				...
				.addTooltip(
						StatCollector
								.translateToLocal(
										"programmable_hatches.gt.piority"))
				.setPos(3 + 2, 18 * 3 + 3 + 1).setSize(16 * 8, 16))
				.widget(new TextFieldWidget().setPattern(BaseTextFieldWidget.WHOLE_NUMS)
						.setGetter(() -> capOverride + "").setSetter(s -> {
							try {
				...
				.widget(new CycleButtonWidget().setToggle(() -> voidFull, val -> {
					...
					if (!voidFull) {
						...
						.setTooltipShowUpDelay(TOOLTIP_DELAY).setPos(3 + 18 * 3, 3 + 18 * 2).setSize(18, 18))
				...
				.widget(new CycleButtonWidget().setToggle(() -> voidOverflow, val -> {
					...
					if (!voidOverflow) {
						...
						.setTooltipShowUpDelay(TOOLTIP_DELAY).setPos(3 + 18 * 4, 3 + 18 * 2).setSize(18, 18));
	...
	public void saveNBTData(NBTTagCompound aNBT) {
		...
		for (int i = 0; i < mInventory.length; i++) {
			...
			if (mInventory[i] != null) {
				...
				if (t != null)
					t.setInteger("Count", mInventory[i].stackSize);
			}
