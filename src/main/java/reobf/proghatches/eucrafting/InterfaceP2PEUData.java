...
public class InterfaceP2PEUData implements AECover.IMemoryCardSensitive, Data, IInterfaceHost, IGridTickable,
    ...
    public void stateChange(final MENetworkChannelsChanged c) {
        this.duality.getInterfaceDuality()
            .notifyNeighbors();
    }
    ...
    public void loadDataFromNBT(NBTBase aNBT) {
        ...
        if (((NBTTagCompound) aNBT).hasKey("realName"))
            duality.setCustomName(((NBTTagCompound) aNBT).getString("realName"));
    ...
    public void update(ICoverable aTileEntity) {
        ...
        Iterator<IGridConnection> it = getProxy().getNode()
            .getConnections()
            .iterator();
        IGridConnection item = null;
        IGridNode thenode = this.duality.getProxy()
            .getNode();
        boolean found = false;
        while (it.hasNext()) {
            item = it.next();
                    ...
                    if (testPart != null && testPart instanceof PartEUP2PInterface) {
                        ...
                        /* if (newBus instanceof PartP2PTunnel<?>newTunnel) */ {
                            ...
                            for (int i = 0; i < duality.getPatterns()
                                .getSizeInventory(); i++) {
                                if (duality.getPatterns()
                                    .getStackInSlot(i) == null) continue;
                                drops.add(
                                    duality.getPatterns()
                                        .getStackInSlot(i));
                                duality.getPatterns()
    ...
    public boolean nonShiftClick(ForgeDirection side, int aCoverID, Data aCoverVariable, ICoverable aTileEntity,
        ...
        if (Optional.ofNullable(aPlayer.getHeldItem())
            .map(ItemStack::getItem)
            .filter(S -> S instanceof IMemoryCard)
            .isPresent()) {
            if (click(aPlayer)) {
                return true;
            }
    ...
    public long doOutput(long aVoltage, long aAmperage) {
        ...
        ICoverable cble = (ICoverable) this.pos.getWorld()
            .getTileEntity(pos.x, pos.y, pos.z);

        if (cble.inputEnergyFrom(side)) return cble.injectEnergyUnits(side, aVoltage, aAmperage);
        return 0;
    }
    ...
    public void addUIWidgets(Builder builder, CoverUIBuildContext ss) {
        ...
        if (ss.isAnotherWindow() && (hasAEGUI() && !ss.getPlayer()
        ...
        if (!tmpout) duality.addWidgets(builder, 16, ss);
        else duality.addWidgetsOut(builder, 16, ss);
        ...
        if (ss.isAnotherWindow() == false) {
            ...
            builder.widget(new ButtonWidget().setOnClick((clickData, widget) -> {
                ...
                .setBackground(GTUITextures.BUTTON_STANDARD, ICON)
                .addTooltips(
                    ImmutableList.of(LangManager.translateToLocalFormatted("programmable_hatches.cover.ae.configure")))
                .setSize(16, 16)
    ...
    private ModularWindow createWindow(EntityPlayer ss, int rgb) {
        ...
        builder.widget(
            ((CycleButtonWidget) new CoverCycleButtonWidget().setSynced(true, true)).setGetter(() -> delay ? 1 : 0)
