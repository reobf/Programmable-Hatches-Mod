...
public class CoverToMachineAdaptor<T extends TileEntity & ICoverable> extends InventoryAdaptor {
    ...
    public CoverToMachineAdaptor(T te, @Nullable InventoryAdaptor invItems, @Nullable IFluidHandler invFluids,
        ForgeDirection fd) {
    ...
    private int checkItemFluids(IFluidHandler tank, InventoryAdaptor inv, ForgeDirection direction) {
        if (tank == null && inv == null) return 2;
        ...
        if (tank != null && tank.getTankInfo(direction) != null) {
            ...
            if (!hasTank && inv == null) return 2;
        }
        if (isGTMachine(tank)) return gtMachineCircuitCheck(inv);
    ...
    private int gtMachineCircuitCheck(InventoryAdaptor ad) {
        ...
        for (ItemSlot i : ad) {
            ...
            if (is == null || Objects.requireNonNull(is.getItem())
                .getUnlocalizedName()
                .equals("gt.integrated_circuit")) continue;
            return 1;
        }
        return 0;
    }
    ...
    public ItemStack addItems(ItemStack toBeAdded, InsertionMode insertionMode) {
        ...
        if (fluid != null) {
            ...
            fluid.amount -= filled;
            return packetOrDrop ? ItemFluidPacket.newStack(fluid) : ItemFluidDrop.newStack(fluid);
        } else {
