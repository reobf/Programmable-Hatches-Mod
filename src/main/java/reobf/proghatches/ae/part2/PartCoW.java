...
public class PartCoW extends PartBasicState implements IGridTickable, IPartGT5Power, IPowerChannelState,IUpdatable {
	...
	public void setInWorld(AENetworkProxy w) {
		...
		try {
			...
			if (f == null) {
				f = w.getClass().getDeclaredField("worldNode");
	...
	public MTECable get() {
		TileEntity te = this.host.getTile().getWorldObj().getTileEntity(this.host.getLocation().x + this.side.offsetX,
				this.host.getLocation().y + this.side.offsetY, this.host.getLocation().z + this.side.offsetZ);
	...
	public void onNeighborChanged() {
		MTECable c = get();
		if (c != null) {
			if(!c.isConnectedAtSide(this.side.getOpposite())){return;}
	...
	public void updateConn() {
		MTECable c = get();
		if (c != null&&c.isConnectedAtSide(this.getSide().getOpposite())) {
			BaseMetaPipeEntity pipe = (((BaseMetaPipeEntity) c.getBaseMetaTileEntity()));
			ArrayList<PartCoW> list = new ArrayList<>();
			final Node node = pipe.getNode();
			if (node != null)
				for (ConsumerNode n : node.mConsumers) {
					...
					if (te instanceof TileCableBus) {
						...
						if (pt instanceof PartCoW && pt != this) {
							if (((PartCoW) pt).channel == this.channel && channel != 0)
								list.add((PartCoW) pt);
						}
			...
			if (connection != null) {
				other = (PartCoW) connection.getOtherSide(this.getGridNode()).getMachine();
				...
				if (list.size() > 1) {
					...
					shouldDestroy = true;
				else if (other != null && other.channel == 0)
					shouldDestroy = true;
				else if (other != null && other.channel != channel)
					shouldDestroy = true;
			...
			if (list.size() == 1 && connection == null) {
				...
				try {
					connection = new CoWGridConnection(this.getGridNode(), list.get(0).getGridNode(),
			...
			if(connection!=null){
				PartCoW other = (PartCoW) connection.getOtherSide(this.getGridNode()).getMachine();
				connection.destroy();
				connection = null;
				other.connection = null;
	...
	public static class WailaDataProvider extends appeng.integration.modules.waila.part.BasePartWailaDataProvider {
		...
		public List<String> getWailaBody(IPart part, List<String> currenttip, IWailaDataAccessor accessor,
			...
			try {
				...
				final String freqTooltip = String.format("%X", accessor.getNBTData().getLong("channel"))
						.replaceAll("(.{4})", "$0 ").trim();
