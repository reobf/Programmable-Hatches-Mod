...
public class ChunkTrackingGridCahce implements IChunkTrackingGridCahce {
    ...
    public ChunkTrackingGridCahce(final IGrid g) {
        ...
        if (!g.getPivot()
            .getWorld().isRemote) cacheinst.add((this));
    }
    ...
    public static class ChunkInfo implements Cloneable, Serializable {
        ...
        public boolean equals(Object obj) {
            ...
            return chunkx == other.chunkx && chunky == other.chunky && dim == other.dim;
        }
    ...
    public void onJoin(IGridStorage sourceStorage) {
        ...
        if (tag.getTag("improperlyUnloaded") instanceof NBTTagIntArray == false) {
            return;
        }
        ...
        ((EntityPlayer) p).addChatMessage(
            new ChatComponentTranslation(
                ...
                "X:" + source.xCoord
                    ...
                    + source.zCoord
                    + ",dim:"
                    + source.getWorldObj().provider.dimensionId
    ...
    public void warnAlt(ChunkInfo info, int i) {
        ...
        terminals.forEach(s -> {
            ...
            if (s.mode == s.DIM) {
                dimensionsToInform.put(s.getWorldObj(), s);
            }
            if (s.mode == s.OWNER) {
                playersUUIDToInform.put(s.owner, s);
            }
        });

        HashSet<EntityPlayer> warned = new HashSet<>();

        List l = MinecraftServer.getServer()
        ...
        if (informAll.isEmpty() == false) for (EntityPlayer p : player) {
    ...
    public void warn(ChunkInfo info, int i) {
        ...
        terminals.forEach(s -> {
            ...
            if (s.mode == s.DIM) {
                dimensionsToInform.add(s.getWorldObj());
            }
            if (s.mode == s.OWNER) {
                playersUUIDToInform.add(s.owner);
            }
        });
        HashSet<EntityPlayer> playersToInform = new HashSet<>();
        if (informAll[0] == false) if (playersUUIDToInform.isEmpty() == false) {
            List l = MinecraftServer.getServer()
                .getConfigurationManager().playerEntityList;
            List<EntityPlayer> player = l;
            for (EntityPlayer p : player) {
        ...
        if (informAll[0] == false) dimensionsToInform.forEach(s -> { playersToInform.addAll(s.playerEntities); });

        if (informAll[0]) {
            playersToInform.addAll(
                MinecraftServer.getServer()
                    .getConfigurationManager().playerEntityList);
        }

        for (Object o : playersToInform) {
            if (((EntityPlayer) o).getEntityWorld().provider.dimensionId == info.dim) ((EntityPlayer) o).addChatMessage(
                new ChatComponentTranslation(
                    "proghatch.chunk_loading_alert.alert",
    ...
    public static void merge(ChunkTrackingGridCahce cache, ChunkTrackingGridCahce cache2) {
        if (cache.improperlyUnloaded.isEmpty()) cache.improperlyUnloaded.addAll(cache2.improperlyUnloaded);
        else if (cache2.improperlyUnloaded.isEmpty()) cache2.improperlyUnloaded.addAll(cache.improperlyUnloaded);
    }
