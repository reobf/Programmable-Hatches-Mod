...
/* 
  ...
  */
		...
		public boolean recordRecipeOrClassify(ListeningFluidTank[] fin, ItemStack[] iin) {
			...
			if(readyToRecord==2){return false;}
			// clearRecipeIfNeeded();
	...
	protected ModularWindow createWindow(final EntityPlayer player, int index) {
		...
		final IItemHandlerModifiable inventoryHandler = new MappingItemHandler(inv0.get(index).mStoredItemInternal, 0,
...

		{
			...
			builder.widget(
					scrollable.setSize(18 * fluidSlotsPerRow(), 18 * Math.min(4, inv0.get(index).mStoredFluidInternal.length)
		...
		builder.widget(TextWidget.dynamicString(() -> inv0.get(index).recipeLocked ? "§4Lock" : "§aIdle").setSynced(true)
		 ...
		 */
		builder.widget(new FakeSyncWidget.StringSyncer(() -> inv0.get(index).toTag().toString(), s -> inv0.get(index).fromTag(cv(s))));
	...
	public void saveNBTData(NBTTagCompound aNBT) {
		...
		for(Entry<Recipe, Integer> e:detailmap.entrySet()){
			...
			aNBT.setTag("detailmap_k"+i, key);i++;
		}
	...
	public  static class Recipe{
		...
		public boolean equals(Object obj) {
			if(obj instanceof Recipe){
				...
				for(int j=0;j<f.length;j++){
					if(!ItemStack.areItemStacksEqual(i[j], p.i[j])){return false;}
					if(!fluidEquals(f[j], p.f[j])){return false;}
				}
				return true;
			}
		...
		public NBTTagCompound ser(){
			...
			for (int i = 0; i < f.length; i++) {
				if (f[i] != null)
					tag.setTag("f" + i,
							f[i].writeToNBT(new NBTTagCompound()));
			}
			tag.setInteger("ff", f.length);
			for (int ii = 0; ii < i.length; ii++) {
				if (i[ii] != null)
					tag.setTag("i" + i,
							writeToNBT(i[ii], new NBTTagCompound()));
	...
	public  class PatternDualInv implements IDualInputInventory{
        public BufferedDualInputHatch parent(){return BufferedDualInputHatch.this;}
		@Nullable DualInvBuffer wrapped;
		final int ID;
	...
	private static class RecipeTracker {
		...
		public void track(@Nonnull FluidTank recipe, @Nonnull FluidTank storage) {
			if (recipe.getFluid().getFluid() != Optional.of(storage).map(FluidTank::getFluid).map(FluidStack::getFluid)
					.orElse(null)) {
				broken = true;
				onceCompared = true;
				return;
	...
	public Iterator<? extends IDualInputInventory> mergeSame() {
		...
		inv0.stream().filter((DualInvBuffer::isAccessibleForMulti)).forEach(s -> {
			a.put(new Wrapper(s), s);
		});
		return (Iterator<? extends IDualInputInventory>) a.asMap().values().stream().map(s -> {
			if (s.size() == 1) {
			...
			for(DualInvBuffer ss:s){
				...
				if(sharedID==0)sharedID=ss.PID;
					...
					if(sharedID!=ss.PID){sharedID=0;break;}
				}
			}
			...
			return new INeoDualInputInventory() {
				...
				public boolean equals(Object obj) {
					if(obj instanceof DualInvBuffer){
						return PID==((DualInvBuffer)obj).PID
								&&
								hashCodef==obj.hashCode();
					}
					return super.equals(obj);
				}
	 ...
	 */
	 ...
	 public void resetMulti() {if(fail){return;}
		 for (ProcessingLogic pl : processingLogics) {
			 for(int i:detailmap.values()){
				 
				 try {
				 if(m==null)
					m=ProcessingLogic.class.getDeclaredMethod("clearCraftingPatternRecipeCache", IDualInputInventory.class);
