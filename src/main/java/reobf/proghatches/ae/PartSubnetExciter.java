...

{
   ...
   @MENetworkEventSubscribe
    public void change(MENetworkChannelChanged w){ gridChanged();}
   @MENetworkEventSubscribe
   public void change(MENetworkChannelsChanged w){gridChanged();}
   @MENetworkEventSubscribe
   public void change(MENetworkControllerChange w){gridChanged();}
    ...
    public void check(@Nullable IGridNode exept/*this node is not checked*/){
    	...
    	try{
    		ok=true;
    	for(IGridNode n:this.getProxy().getGrid().getNodes()){
    		if(n==exept)continue;
    	...
    	if(prevok!=ok){try{
    		...
    		this.getProxy().getGrid().postEvent(new MENetworkPowerStatusChange());
    		this.getProxy().getGrid().postEvent(new MENetworkPowerStorage(this, PowerEventType.PROVIDE_POWER));
    		World w = getHost().getTile().getWorldObj();
    		if(!ok)w .playSoundEffect((double)(getHost().getTile().xCoord + 0.5F), (double)(getHost().getTile().yCoord + 0.5F), (double)(getHost().getTile().zCoord + 0.5F), "random.fizz", 0.5F, 2.6F + (w.rand.nextFloat() - w.rand.nextFloat()) * 0.8F);
    		//if(ok)w .playSoundEffect((double)(getHost().getTile().xCoord + 0.5F), (double)(getHost().getTile().yCoord + 0.5F), (double)(getHost().getTile().zCoord + 0.5F), "random.orb", 0.5F, 2.6F + (w.rand.nextFloat() - w.rand.nextFloat()) * 0.8F);

    		ChannelLoader.sendPacketToAllPlayers(this.host.getTile().getDescriptionPacket(), this.host.getTile().getWorldObj());
    ...
    public void renderDynamic(double x, double y, double z, IPartRenderHelper rh, RenderBlocks renderer) {
    	...
    	if(prevok==true&&ok==false){
    		...
    		Minecraft.getMinecraft().theWorld.spawnParticle("smoke", 	
    				 this.getHost().getTile().xCoord+0.5+this.getSide().offsetX*0.5
    					+ran.nextDouble()*0.3-ran.nextDouble()*0.3,
    					this.getHost().getTile().yCoord+0.5+this.getSide().offsetY*0.5
    					+ran.nextDouble()*0.3-ran.nextDouble()*0.3,
    					this.getHost().getTile().zCoord+0.5+this.getSide().offsetZ*0.5
    					+ran.nextDouble()*0.3-ran.nextDouble()*0.3, 0.0f, 0.0f, 0.0f);
      ...
      if(prevok==false&&ok==true){
    		...
    		for(int i=0;i<16;i++)
    		Minecraft.getMinecraft().theWorld.spawnParticle("reddust", 	
    				 this.getHost().getTile().xCoord+0.5+this.getSide().offsetX*0.5
    					+ran.nextDouble()*0.3-ran.nextDouble()*0.3,
    					this.getHost().getTile().yCoord+0.5+this.getSide().offsetY*0.5
    					+ran.nextDouble()*0.3-ran.nextDouble()*0.3,
    	...
    	prevok=ok;
    	final ChargedOreFX fx = new ChargedOreFX(Minecraft.getMinecraft().theWorld, 
				
				this.getHost().getTile().xCoord+0.5+this.getSide().offsetX*0.5
				+ran.nextDouble()*0.2-ran.nextDouble()*0.2,
				this.getHost().getTile().yCoord+0.5+this.getSide().offsetY*0.5
				+ran.nextDouble()*0.2-ran.nextDouble()*0.2,
				this.getHost().getTile().zCoord+0.5+this.getSide().offsetZ*0.5
				+ran.nextDouble()*0.2-ran.nextDouble()*0.2
