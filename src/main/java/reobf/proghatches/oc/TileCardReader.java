...
public class TileCardReader extends TileEntity implements ITileWithModularUI,IInventory,ISidedInventory,li.cil.oc.api.network.Environment{
	...
	public Object[] get(int index,int line,boolean lab,boolean concat){
		...
		((IPanelDataSource)is.getItem()).update(worldObj, w, Integer.MAX_VALUE);PanelString str;
		try{
		 str = ((IPanelDataSource)is.getItem()).getStringData(-1, w, lab).get(line)
	...
	Node node= li.cil.oc.api.Network.newNode(this, Visibility.Network).withComponent("cardreader").create();
	...
	public ModularWindow createWindow(UIBuildContext buildContext) {
		...
		IDrawable slot=new AdaptableUITexture(new ResourceLocation("nuclearcontrol","textures/gui/GUIAdvancedInfoPanel.png")
			,7/256.0f
			,41/256.0f
			,(7+18)/256.0f
			,(41+18)/256.0f
		...
		final IDrawable[] background = new IDrawable[] { slot/*GUITextureSet.DEFAULT.getItemSlot()*/ };
			...
			builder.widget(SlotGroup.ofItemHandler(new MappingItemHandler(inv, 0, 27), 9)

					.startFromSlot(0).endAtSlot(26).background(background)
					...
					.build().setPos(3+4 , 3+4).setSize(18 * 9, 18 * 3)
	...
	public void updateEntity() {
		if (init == false) if (node != null) {init=true;
