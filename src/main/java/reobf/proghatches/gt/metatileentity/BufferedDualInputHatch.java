...

{
    ...
    public void initTierBasedField() {
        ...
        /*
         * if (mMultiFluid) { mStoredFluid = new ListeningFluidTank[] {
         * new ListeningFluidTank((int) (1000 * Math.pow(2, mTier)), this), new
         * ListeningFluidTank((int) (1000 * Math.pow(2, mTier)), this), new
         * ListeningFluidTank((int) (1000 * Math.pow(2, mTier)), this), new
         * ListeningFluidTank((int) (1000 * Math.pow(2, mTier)), this)
         * }; } else {
    ...
    public class DualInvBuffer implements INeoDualInputInventory {
        ...
        public void fromTag(NBTTagCompound tag) {
            ...
            if (i == 0) if (tag.getInteger("i") > 0) i = tag.getInteger("i");
            if (f == 0) if (tag.getInteger("f") > 0) f = tag.getInteger("f");
        ...
        public int clearRecipeIfNeeded() {
            ...
            if (isEmpty()) {
                ...
                /*
                 * if(detail!=null){
                 * int index=inv0.indexOf(this);
                 * DualInvBuffer neo=new DualInvBuffer();
                 * neo.init(mInventory.length - 1, mStoredFluid.length);
                 * DualInvBuffer old = inv0.set(index,neo);
                 * return 2;
                 * }
                 */
        ...
        public boolean classify(ListeningFluidTank[] fin, ItemStack[] iin, boolean removeInputOnSuccess) {
            ...
            for (int ix = 0; ix < f; ix++) {
                ...
                if (removeInputOnSuccess) fin[ix].setFluidDirect(null);
                else if (fin[ix].getFluid() != null) fin[ix].setFluidDirect(
                    fin[ix].getFluid()
                        .copy());
        ...
        public ItemStack[] getItemInputs() {
             ...
             * (additional == null) return condensed;
             * int before_size = condensed.length; ItemStack[] bruh = new
             * ItemStack[before_size + 1]; bruh[before_size] = additional;
             * System.arraycopy(condensed, 0, bruh, 0, before_size); return
             * bruh;
             */
    ...
    public void onPostTick(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
        ...
        toDisconnect.forEach(s -> { s.wrapped = null; });
        toDisconnect.clear();
        // System.out.println(scheduled);
        // System.out.println(aTick+" "+scheduled.peekLast());
        Optional.ofNullable(scheduled.peekLast())
            .filter(s -> s < aTick)
            .ifPresent(s -> {
        ...
        boolean on = (this.getBaseMetaTileEntity()
            .isAllowedToWork());
        if (isOnLastTick != on) {
            dirty = true;
        } ;
        ...
        if (dirty) {
            ...
            if (inputEmpty.get()) {
                if (preventSleep == 0) if (Config.sleep) sleep = true;
        ...
        if (!sleep || updateEveryTick()) for (DualInvBuffer inv0 : this.sortByEmpty()) {
        ...
        if (autoAppend && allFull && !isInputEmpty()) {
            ...
            if (inv0.size() > bufferNum) {
                ...
                if (exfull/* &&inv0.size()>bufferNum */) for (int i = 0; i < bufferNum; i++) {
                    if (inv0.get(i)
                        .isEmpty() && (!inv0.get(i).recipeLocked)) {
    ...
    public ArrayList<DualInvBuffer> sortByEmpty() {
        ...
        inv0.forEach(s -> { (s.isEmpty() && (!s.recipeLocked/*
    ...
    public void classify() {
        if (isRemote()) return;
        for (DualInvBuffer inv0 : this.sortByEmpty()) {
            if (inv0.full() == false) if (inv0.classify(this.mStoredFluid, mInventory, true)) break;
        }
    ...
    public DualInvBuffer classifyForce() {
        ...
        for (DualInvBuffer inv0 : this.sortByEmpty()) {
            if (inv0.full() == false) if (inv0.classify(this.mStoredFluid, mInventory, true)
                || inv0.recordRecipeOrClassify(mStoredFluid, mInventory)) return inv0;
        }
        return null;
    ...
    private Widget createButtonBuffer(int id, int xoffset, int yoffset) {
        ...
        return new ButtonWidget().setOnClick((clickData, widget) -> {
            ...
            .setSize(16, 16)
            .setPos(xoffset + 16 * (id % 3), yoffset + 16 * (id / 3));
    ...
    protected ModularWindow createWindow(final EntityPlayer player, int index) {
        ...
        for (int i = 0; i < inv0.get(index).v; i++)
            ...
            sc.widget(
                (i == 0 ? circuitSlot(inventoryHandler, inv0.get(index).i + i)
...

        {
            ...
            for (int i = 0; i < inv0.get(index).mStoredFluidInternal.length; i++) {
                ...
                scrollable.widget(
                    new FluidSlotWidget(new LimitedFluidTank(inv0.get(index).mStoredFluidInternal[i]))
        ...
        builder.widget(
            TextWidget.dynamicString(() -> inv0.get(index).recipeLocked ? "§4Lock" : "§aIdle")
                .setSynced(true)
    ...
    ButtonWidget createPowerSwitchButton(IWidgetBuilder<?> builder) {
        ...
        Widget button = new ButtonWidget().setOnClick((clickData, widget) -> {
            ...
            .setPlayClickSoundResource(
                () -> thiz.isAllowedToWork() ? SoundResource.GUI_BUTTON_UP.resourceLocation
        ...
        builder.widget(new SyncedWidget() {
            ...
            public void detectAndSendChanges(boolean init) {
                ...
                if (getBaseMetaTileEntity() != null)
                    if (last >= getBaseMetaTileEntity().getTimer()) getWindow().getContext()
                        .getContainer().inventorySlots.forEach(s -> ((Slot) s).onSlotChanged());
    ...
    public void loadNBTData(NBTTagCompound aNBT) {
        ...
        while (true) {
            ...
            if (value > 0) {
                ...
                // ((ICraftingPatternItem)is.getItem()).getPatternForItem(is, getBaseMetaTileEntity().getWorld())
                    , value);
            } else {
                break;
            }
    ...
    public static class Recipe {
        ...
        public static Recipe fromBuffer(DualInvBuffer buf, boolean copy) {
            Recipe r = new Recipe();
            r.i = buf.mStoredItemInternalSingle;
            if (copy) r.i = r.i.clone();
            for (int i = 0; i < r.i.length; i++) {
                if (copy) r.i[i] = r.i[i] == null ? null : r.i[i].copy();
            }
            r.f = new FluidStack[buf.mStoredFluidInternalSingle.length];
            for (int i = 0; i < r.f.length; i++) {
                FluidStack fs = buf.mStoredFluidInternalSingle[i].getFluid();
                if (copy && fs != null) fs = fs.copy();
                r.f[i] = fs;
            }
        ...
        public int hashCode() {
            int hashCode = 1;
            for (ItemStack e : i) hashCode = 31 * hashCode + (e == null ? 0 : hashCode(e));
            for (FluidStack e : f) hashCode = 31 * hashCode + (e == null ? 0 : e.hashCode());
            return hashCode;
        }
        ...
        public boolean equals(Object obj) {
            if (obj instanceof Recipe) {
                ...
                for (int j = 0; j < f.length; j++) {
                    if (!ItemStack.areItemStacksEqual(i[j], p.i[j])) {
                        return false;
                    }
                    if (!fluidEquals(f[j], p.f[j])) {
                        return false;
                    }
                }
                return true;
        ...
        public NBTTagCompound ser() {
            ...
            for (int i = 0; i < f.length; i++) {
                if (f[i] != null) tag.setTag("f" + i, f[i].writeToNBT(new NBTTagCompound()));
            }
        ...
        public static Recipe deser(NBTTagCompound tag) {
            Recipe r = new Recipe();
            r.f = new FluidStack[tag.getInteger("ff")];
            r.i = new ItemStack[tag.getInteger("ii")];
            for (int i = 0; i < r.f.length; i++) {
                if (tag.hasKey("f" + i)) {
                    r.f[i] = FluidStack.loadFluidStackFromNBT(tag.getCompoundTag("f" + i));
                }
            }
            for (int i = 0; i < r.i.length; i++) {
                if (tag.hasKey("i" + i)) {
                    r.i[i] = loadItemStackFromNBT(tag.getCompoundTag("i" + i));
                }
            }
            ...
            .forEach(s -> {
                ...
                sub.setString(
                    ...
                    IntStream.range(0, inv.mStoredFluidInternalSingle.length)
                        ...
                        .filter(ss -> ss.holded.getFluidAmount() > 0)
                        .map(ss -> {
    ...
    public long getPossibleCopies(DualInvBuffer toCheck) {
        ...
        IntStream.range(0, inv.mStoredItemInternalSingle.length)
            .mapToObj(ss -> new IndexedObject<>(ss, inv.mStoredItemInternalSingle[ss]))
            .filter(ss -> ss.holded != null)
            .forEach(ss -> { rt.track(ss.holded, inv.mStoredItemInternal[ss.index]); });

        IntStream.range(0, inv.mStoredFluidInternalSingle.length)
            .mapToObj(ss -> new IndexedObject<>(ss, inv.mStoredFluidInternalSingle[ss]))
            .filter(ss -> ss.holded.getFluidAmount() > 0)
            .forEach(ss -> { rt.track(ss.holded, inv.mStoredFluidInternal[ss.index]); });
    ...
    public void getWailaBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (Config.debug || Config.dev) currenttip.add(
        ...
        IntStream.range(0, tag.getInteger("inv_size"))
            .forEach(s -> {
                ...
                if (copies > 0) {
                    cpinfo = cpinfo + LangManager
                        .translateToLocalFormatted("programmable_hatches.buffer.waila.copies", copies + "");
                ...
                if (!lock_item.isEmpty()) Arrays.stream(lock_item.split("\n"))
                    .map(ss -> " " + ss)
                    .forEach(currenttip::add);
                if (!lock_fluid.isEmpty()) Arrays.stream(lock_fluid.split("\n"))
                    .map(ss -> " " + ss)
                    .forEach(currenttip::add);

            });;
        if (idle[0] > 5) currenttip
            .add(LangManager.translateToLocalFormatted("programmable_hatches.buffer.waila.hidden", (idle[0] - 5) + ""));
        if (tag.getInteger("exinvlen") > 0) currenttip.add("Extra buffer:" + tag.getInteger("exinvlen"));
    }

    private Boolean isRemote;

    public boolean isRemote() {
        if (isRemote == null) isRemote = FMLCommonHandler.instance()
            .getEffectiveSide() == Side.CLIENT;// this.getBaseMetaTileEntity().getWorld().isRemote;
        return isRemote;
    }
        ...
        if (tile.isServerSide()) {
            if (!tile.privateAccess() || aPlayer.getDisplayName()
                ...
                if (tCurrentItem != null) {
                    ...
                    for (int id : OreDictionary.getOreIDs(tCurrentItem)) {
                        if (OreDictionary.getOreName(id)
                            .equals(ToolDictNames.craftingToolFile.toString())) {
                            suc = true;
                            break;
                        } ;
                    }
                ...
                if (tCurrentItem != null) {
                    ...
                    for (int id : OreDictionary.getOreIDs(tCurrentItem)) {
                        if (OreDictionary.getOreName(id)
                            .equals(ToolDictNames.craftingToolSaw.toString())) {
                            suc = true;
                            break;
                        } ;
                    }
                    if (suc) {
                        ...
                        GTUtility.sendSoundToPlayers(
                            tile.getWorld(),
                            SoundResource.IC2_TOOLS_CHAINSAW_CHAINSAW_USE_TWO,
                            1.0F,
                            -1,
                            tile.getXCoord(),
                            tile.getYCoord(),
                        ...
                        /*
                         * merge = !merge;
                         * GTUtility.sendChatToPlayer(aPlayer, "merge:" +
                         * merge);
...
*/
    static public boolean fluidEquals(FluidTank a, FluidTank b) {
        ...
        if (a.getFluid() == null && b.getFluid() == null) return true;
        if (a.getFluid() != null && (!a.getFluid()
            .equals(b.getFluid()))) return false;
    ...
    protected ModularWindow createWindowEx(final EntityPlayer player) {
        ...
        builder.widget(new CycleButtonWidget().setToggle(() -> useNewGTPatternCache, (s) -> {
            ...
            .addTooltip(
                (MyMod.newGTCache) ? "" : StatCollector.translateToLocal("programmable_hatches.gt.newcrib.nosupport"))
    ...
    public void resetMulti() {
        ...
        for (ProcessingLogic pl : processingLogics) {
            for (int i : detailmap.values()) {
                ...
                try {
                    if (m == null) m = ProcessingLogic.class
                        .getDeclaredMethod("clearCraftingPatternRecipeCache", IDualInputInventory.class);
