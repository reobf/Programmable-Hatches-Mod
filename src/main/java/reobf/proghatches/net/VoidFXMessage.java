...
public class VoidFXMessage implements IMessage {
    ...
    public static class Handler implements IMessageHandler<VoidFXMessage, VoidFXMessage> {
        ...
        public VoidFXMessage onMessage(VoidFXMessage message, MessageContext ctx) {
            ...
            try {
                ...
                if (message.id != -1) try {
                    ((VoidOutputHatch) ((IGregTechTileEntity) Minecraft.getMinecraft().theWorld
                        .getTileEntity(message.x, message.y, message.z)).getMetaTileEntity())
                            .addVisual(new FluidStack(FluidRegistry.getFluid(message.id), message.amount));
                    ...
                    ((VoidOutputBus) ((IGregTechTileEntity) Minecraft.getMinecraft().theWorld
                        .getTileEntity(message.x, message.y, message.z)).getMetaTileEntity()).addVisual(message.is);
    ...
    public void fromBytes(ByteBuf buf) {
        ...
        if (id == -1) try {
            is = ItemStack
                .loadItemStackFromNBT(CompressedStreamTools.func_152457_a(b, new NBTSizeTracker(Long.MAX_VALUE)));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void toBytes(ByteBuf buf) {
        ...
        buf.writeInt(amount);
        if (is != null) try {
            byte[] b = CompressedStreamTools.compress(is.writeToNBT(new NBTTagCompound()));
