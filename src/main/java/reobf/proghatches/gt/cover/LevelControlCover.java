...
public ModularWindow createWindow(GT_CoverUIBuildContext buildContext) {
		...
		private ItemStack tryConvertToFluid(ItemStack is){
		 ...
		 protected void addUIWidgets(ModularWindow.Builder builder) {
			 ...
			 ItemStackHandler iss=new ItemStackHandler(getCoverData().filter);
				...
				}.setPos(3, 3+20).addTooltip(StatCollector.translateToLocal("proghatches.amountmaintainer.phantomslot")));
	...
	protected Data doCoverThingsImpl(ForgeDirection side, byte aInputRedstone, int aCoverID, Data aCoverVariable,
		...
		if(grid==null){
		   ...
		   IAEFluidStack avf = grid.getStorage().getFluidInventory().getAvailableItem(
						aCoverVariable.maybeFluid()
						);
			 amount=avf==null?0:avf.getStackSize();
				
				if(amount==0){
			IAEItemStack av = grid.getStorage().getItemInventory().getAvailableItem(
			aCoverVariable.maybeItem()
			);
