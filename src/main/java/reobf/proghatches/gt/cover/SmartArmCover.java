...
public class SmartArmCover extends GT_CoverBehaviorBase<SmartArmCover.Data> {
    ...
    static public class Data implements ISerializableObject {
        ...
        public NBTBase saveDataToNBT() {
            ...
            t.setString(
                "formulaprev",
                Optional.ofNullable(formulaprev)
                    .orElse(""));
            ...
            Optional.ofNullable(e)
                .map(json::toJson)
                .ifPresent(s -> t.setByteArray("expression", s.getBytes()));
            // json.toJson(e);
        ...
        public void loadDataFromNBT(NBTBase aNBT) {
            ...
            Optional.ofNullable(((NBTTagCompound) aNBT).getByteArray("expression"))
                .map(s -> json.fromJson(new String(s), Expression.class))
                .ifPresent(s -> e = s);
    ...
    private class ArmUIFactory extends UIFactory {
        ...
        protected ModularWindow createWindow(final EntityPlayer player) {
            ...
            builder.setGuiTint(
                this.getUIBuildContext()
                    .getGuiColorization());
            builder.setDraggable(true);
            builder.widget((new com.gtnewhorizons.modularui.common.widget.ButtonWidget()).setOnClick((s, b) -> {
                if (getCoverData().state != 0) return;
                if (s.mouseButton == 0) getCoverData().probe++;
                else getCoverData().probe--;
                int len = getCoverData().key.length;
                if (len > 0) if (getCoverData().probe >= len) {
        ...
        protected void addUIWidgets(ModularWindow.Builder builder) {
            maxSlot = getMaxSlot();
            // CoverDataControllerWidget<Data> o = new CoverDataControllerWidget<>(this::getCoverData,
            ...
            builder.widget(
                (new com.gtnewhorizons.modularui.common.widget.ButtonWidget())
            ...
            CycleButtonWidget c;
            builder.widget(
                (c = (CycleButtonWidget) new CoverCycleButtonWidget().setSynced(false, true))
                    .setGetter(() -> getCoverData().dyn ? 1 : 0)
                    .setSetter(s -> {
            ...
            builder.widget(
                Optional.of(new TextFieldWidget())
                    ...
                    .setGetter(() -> { return getCoverData().formula; })
                    .setSetter(s -> { getCoverData().formula = s; })
                    ...
                    .setPos(startX + spaceX, startY + spaceY * 2)
                    .setSize(spaceX * 3, 12)
            ...
            /*
             * o.addFollower(
             * new CoverDataFollower_CycleButtonWidget<Data>(),
             * (s)->{return (s.io)?1:0;},
             * (b,s)->{
             * b.io=s==1;
             * return b;},
            ...
            UITexture.fullImage(new ResourceLocation("proghatches", "textures/formula" + getCoverData().mode + ".png"))
                ...
                .widget(
                    new TextWidget("f =").setDefaultColor(COLOR_TEXT_GRAY.get())
                        .setPos(startX, startY + spaceY * 2))

                .widget(
                    TextWidget.dynamicString(() -> { return getCoverData().detail; })
                        .setSynced(false)
                        .setTextAlignment(Alignment.CenterLeft)
                        .setDefaultColor(COLOR_TEXT_GRAY.get())
    ...
    protected boolean onCoverRightClickImpl(ForgeDirection side, int aCoverID, Data d, ICoverable aTileEntity,
        ...
        if (1 > 0) return super.onCoverRightClickImpl(side, aCoverID, d, aTileEntity, aPlayer, aX, aY, aZ);
        // not used
            ...
            if (d.state == 2) {
                ...
                if (d.cslot < argsize) {
                    ...
                    try {
                        // System.out.println(
                        to = (int) Math.round(
                            ((Number) d.e.evaluate(new Context(publicContext).add(d.mode == 1 ? "i" : "o", d.cslot)))
                                .doubleValue());
                    } catch (Exception e) {
                ...
                if (d.dyn == false) {
                    ...
                    if (d.io) {// I->O
                        for (int i = 0; i < d.key.length && remain > 0; i++)
                            /* if(thiz.getStackInSlot(in.applyAsInt(i))!=null) */remain -= 0
                                < ProghatchesUtil.moveFromSlotToSlotSafe(
                                    thiz,
                                    that,
                                    in.applyAsInt(i),
                        ...
                        for (int i = 0; i < d.key.length && remain > 0; i++)
                            /* if(that.getStackInSlot(out.applyAsInt(i))!=null) */remain -= 0
                                < ProghatchesUtil.moveFromSlotToSlotSafe(
                                    that,
                                    thiz,
                                    out.applyAsInt(i),
    ...
    private Context addDyn(Context add, Data d, ICoverable aTileEntity) {
        ...
        add.add("redstone", (sp) -> {
            ...
            byte b = ProghatchesUtil.getSignalAt(
                extra == 1 ? ((IGregTechTileEntity) aTileEntity).getOwnerUuid() : null,
                freq,
                GateMode.valueOf(extra2));
    ...
    {/*
      * publicContext.add("pow", s->{
      * Number a=(Number) s.get(0);
      * Number b=(Number) s.get(1);
      * fail:if(a instanceof SimpleParser.Rational&&a instanceof SimpleParser.Rational){
    ...
    public String probe(Data d, ICoverable c) {
        if (d.state != 0 || d.formula.isEmpty()) return "";
        try {
            if (d.dyn) {
