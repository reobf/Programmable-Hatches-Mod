...
public class ItemGTRedstoneCard extends Item implements li.cil.oc.api.driver.item.HostAware {
    ...
    public class RedstoneEnv implements ManagedEnvironment {
        ...
        // public RedstoneEnv(EnvironmentHost env){this.env=env;};EnvironmentHost env;
        private Node _node = Network.newNode(this, Visibility.Network)
            .withComponent("gtredstone")
        ...
        @Callback(
            doc = "getWireless(frequency:number[,isPublic:boolean]) --get GT wireless redstone, 'isPublic' defaults to true",
            direct = false)
        ...
        @Callback(
            doc = "setWireless(frequency:number,signal:number[,isPublic:boolean]) --set GT wireless redstone, 'isPublic' defaults to true",
            direct = false)
        ...
        @Callback(
            doc = "setGateMode(mode:string) --set GateMode, available modes: AND,NAND,OR,NOR,SINGLE_SOURCE",
            direct = false)
        public Object[] setGateMode(final Context context, final Arguments args) {

            // try{
            setMode(stack, GateMode.valueOf(args.checkString(0)));
            // }catch(Exception e){throw new RuntimeException("Failed, check your args.");}
            return null;
        }
        ...
        @Callback(
            doc = "clearWireless(isPublic:boolean[,frequency:number]) --clear GT wireless redstone, will clear all signals emitted by this card if 'frequency' is absent.",
            direct = false)
