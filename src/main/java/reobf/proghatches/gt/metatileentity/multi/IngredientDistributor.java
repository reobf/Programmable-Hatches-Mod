...
public class IngredientDistributor extends MTEEnhancedMultiBlockBase<IngredientDistributor>
    ...
    public boolean addOutputBusToMachineList(IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
        ...
        if (ok && !hasBusThisLayer) {
            hasBusThisLayer = true;
        } else {
            return false;
        }
        if (ok && !(aTileEntity.getMetaTileEntity() instanceof MTEHatchOutputBusME)) {
            allMEHatch = false;
        }
        return ok;
    };
    ...
    public boolean addOutputHatchToMachineList(IGregTechTileEntity aTileEntity, int aBaseCasingIndex) {
        ...
        if (ok && !hasHatchThisLayer) {
            hasHatchThisLayer = true;
        } else {
            return false;
        }
        if (ok && !(aTileEntity.getMetaTileEntity() instanceof MTEHatchOutputME)) {
            allMEHatch = false;
        }
    ...
    IStructureDefinition<IngredientDistributor> STRUCTURE_DEFINITION = StructureDefinition
        .<IngredientDistributor>builder()
        .addShape(
            "first",
            StructureUtility.transpose(new String[][] { { "m", "f", "c" }, { "m", "f", "c" }, { "m", "f", "c" } }))
        .addShape(
            "second",
            StructureUtility.transpose(new String[][] { { "m", "f", "c" }, { "~", "c", "c" }, { "m", "c", "c" } }))
        .addShape(
            "third",
            StructureUtility.transpose(new String[][] { { "m", "f", "c" }, { "m", "c", "c" }, { "◎", "c", "c" } }))
        .addShape(
            "piece",
            StructureUtility.transpose(new String[][] { { " ", " ", " " }, { " ", "f", " " }, { "h", "h", "h" } }))
        .addShape(
            "piece_survival",
            StructureUtility.transpose(new String[][] { { " ", " ", " " }, { " ", "f", " " }, { "h", "※", "h" } }))

        .addShape(
            "last",
            StructureUtility.transpose(new String[][] { { " ", " ", " " }, { " ", "f", " " }, { " ", "f", " " } }))
            ...
            buildHatchAdder(IngredientDistributor.class).atLeast(HatchElement.Maintenance, HatchElement.Energy)
                ...
                .buildAndChain(
                    ...
                    buildHatchAdder(IngredientDistributor.class).atLeast(new IHatchElement<IngredientDistributor>() {
                        ...
                        .hatchItemFilter(s -> (
                        // have to return this or will this not be
                        // displayed in NEI 'availale hatches' list
                        is -> is.getItemDamage()
    ...
    public boolean checkMachine(IGregTechTileEntity aBaseMetaTileEntity, ItemStack aStack) {
        isLiteVersion = false;
        port = null;
        if (checkPiece("first", 1, 1, 0) && checkPiece("second", 0, 1, 0) && checkPiece("third", -1, 1, 0)) {} else {
            return false;
        }
        int index = -2;
    ...
    /*
     * protected void drawTexts(DynamicPositionedColumn screenElements,
     * SlotWidget inventorySlot) { super.drawTexts(screenElements,
     * inventorySlot); screenElements.widget( TextWidget.dynamicString(()->
     * StatCollector.translateToLocalFormatted("proghatches.ingdistr.eu",
     * cost(),getMaxInputEu()) ).setDefaultColor(COLOR_TEXT_WHITE.get())
     * .setEnabled(widget -> { return
    ...
    protected void runMachine(IGregTechTileEntity aBaseMetaTileEntity, long aTick) {
        ...
        if (ready > 0 && aBaseMetaTileEntity.isAllowedToWork()) {
            ...
            if (port != null && !lockRecipe
                && isAllMEOutputEmpty() == true
                && (!port.getBaseMetaTileEntity()
                    .getRedstone() || count > 0)
                && !isInputEmpty()) {
                count++;
                porton = true;
    ...
    /*
     * @SuppressWarnings({ "unchecked", "rawtypes", "unused" }) private
     * Optional<IRecipeProcessingAwareHatch> getInput(){
     * if(mDualInputHatches.size()==1){ return
     * (Optional)Optional.ofNullable(mDualInputHatches.get(0)).filter(s->s
     * instanceof IRecipeProcessingAwareDualHatch) .map(s->new
     * IRecipeProcessingAwareHatch(){ public void startRecipeProcessing() {
     * ((IRecipeProcessingAwareDualHatch)s).startRecipeProcessing(); } public
     * CheckRecipeResult endRecipeProcessing(MTEMultiBlockBase controller) {
     * return
     * ((IRecipeProcessingAwareDualHatch)s).endRecipeProcessing(controller); }})
     * ; } if(mInputBusses.size()==1){ return
     * (Optional)Optional.ofNullable(mInputBusses.get(0)).filter(s->s instanceof
     * IRecipeProcessingAwareHatch); } if(mInputHatches.size()==1){ return
     * (Optional)Optional.ofNullable(mInputHatches.get(0)).filter(s->s
     * instanceof IRecipeProcessingAwareHatch); } return Optional.empty(); }
     */
    ...
    public boolean isInputEmpty() {
        ...
        try {
            if (mDualInputHatches.size() == 1) {
                return !mDualInputHatches.get(0)
                    .getFirstNonEmptyInventory()
                    .isPresent();
            }
            if (mInputBusses.size() == 1) {
                MTEHatchInputBus bus = mInputBusses.get(0);
                for (int i = 0; i < bus.getSizeInventory(); i++) {
                    if (bus.getStackInSlot(i) != null || bus.getStackInSlot(i).stackSize > 0) {
                        return false;
                    }
                }
            }
            if (mInputHatches.size() == 1) {
                ...
                for (int i = 0; i < bus.size(); i++) {
                    if (bus.get(i) != null || bus.get(i).amount > 0) {
                        return false;
                    }
                }
            }
        ...
        try {
            ...
            mDualInputHatches.forEach(s -> {
                if (s instanceof MTEHatchInputBus) {
                    ...
                    MTEHatchInputBus b = (MTEHatchInputBus) s;
                    b.updateSlots();
                }
    ...
    private boolean doDistribute() {
        ...
        try {
            ...
            if (possibleSource == null && mInputHatches.size() == 1) {
                ...
                if (fluid.size() > 0) {
                    possibleSource = ImmutableList.of(new INeoDualInputInventory() {
            ...
            // boolean
            // fail=in.map(s->s.endRecipeProcessing(this)).filter(s->!s.wasSuccessful()).isPresent();
    ...
    private boolean moveToOutpusME(IDualInputInventory opt) {
        ...
        for (int index = 0; index < mOutputBusses.size(); ++index) {
            ...
            TransferCheckResult result = checkMEBus(
                ((MTEHatchOutputBusME) mOutputBusses.get(index)),
        ...
        for (int index = 0; index < mOutputHatches.size(); ++index) {
            ...
            TransferCheckResult result = checkMEHatch(
                ((MTEHatchOutputME) mOutputHatches.get(index)),
        ...
        for (int index = 0; index < i.length; ++index) {
            try {
                IAEItemStack notadded = ((MTEHatchOutputBusME) mOutputBusses.get(index)).getProxy()
                    .getStorage()
                    .getItemInventory()
                    .injectItems(
                        AEApi.instance()
                            .storage()
                            .createItemStack(i[index]),
                        Actionable.MODULATE,
                        getActionSourceFor(mOutputBusses.get(index)));

                i[index].stackSize = Optional.ofNullable(notadded)
        ...
        for (int index = 0; index < f.length; ++index) {
            try {
                IAEFluidStack notadded = ((MTEHatchOutputME) mOutputHatches.get(index)).getProxy()
                    .getStorage()
                    .getFluidInventory()
                    .injectItems(
                        AEApi.instance()
                            .storage()
                            .createFluidStack(f[index]),
                        Actionable.MODULATE,
                        getActionSourceFor(mOutputHatches.get(index)));

                f[index].amount = Optional.ofNullable(notadded)
                    .map(s -> s.getStackSize())
                    .orElse(0l)
                    .intValue();
            } catch (GridAccessException e) {}
        ...
        public String format() {
            ...
            for (int i = 0; i < args.length; i++) {
                ...
                if (args[i] instanceof FluidStack) {
                    args[i] = ((FluidStack) args[i]).getFluid()
                        .getName() + " " + ((FluidStack) args[i]).amount + "L";
                }
        ...
        static TransferCheckResult deser(PacketBuffer pb) {
            try {
                ...
                for (int i = 0; i < len; ++i) {
                    ...
                    if (type == 0) r = tag.getString("o" + i);
                    if (type == 1) r = ItemStack.loadItemStackFromNBT(tag.getCompoundTag("o" + i));
                    if (type == 2) r = AEItemStack.loadItemStackFromNBT(tag.getCompoundTag("o" + i));
                    if (type == 3) r = FluidStack.loadFluidStackFromNBT(tag.getCompoundTag("o" + i));
                    // if(type==4)
            ...
            try {
                ...
                for (int i = 0; i < thiz.args.length; i++) {
                    ...
                    if (o instanceof AEFluidStack) {
                        /*
                         * NBTTagCompound tmp = new NBTTagCompound();
                         * ((AEFluidStack) o).writeToNBT(tmp); tag.setTag("o"+i,
                         * tmp); tag.setInteger("t"+i, 4);
                         */
...
*/
           ...
           // IItemList<IAEItemStack> itemCache = (IItemList<IAEItemStack>) f.get(bus);
            ...
            while (itr.hasNext()) {
                ...
                if ((next = itr.next()).isSameType(check) == false && next.getStackSize() > 0) {
                    if (check == null) return TransferCheckResult.ofFail("cache.diff.bus.null", index, cp(next));
                    return TransferCheckResult.ofFail("cache.diff.bus", index, cp(next), cp(check));
                }
            }
            IMEMonitor<IAEItemStack> itemInventory = bus.getProxy()
                .getStorage()
                .getItemInventory();
            itr = itemInventory.getAvailableItems(
                AEApi.instance()
                    .storage()
                    .createItemList(),
                IterationCounter.fetchNewId())
            ...
            while (itr.hasNext()) {
                ...
                if (next != null) if ((next).isSameType(check) == false && next.getStackSize() > 0) {
            ...
            if (check != null) {
                ...
                IAEItemStack notadded = bus.getProxy()
                    .getStorage()
                    .getItemInventory()
                    .injectItems(AEItemStack.create(check), Actionable.SIMULATE, getActionSourceFor(bus));

                if (notadded != null && notadded.getStackSize() > 0) return TransferCheckResult.ofFail(
                    "inject.failure." + (bus.getProxy()
                        .isPowered()
                        && bus.getProxy()
                            .isActive())
                        + ".bus",
        ...
        try {
          ...
          //  Iterator<IAEFluidStack> itr = itemCache.iterator();
            ...
            while (itr.hasNext()) {
                ...
                if (next != null) if (!sameType(next, (check)) && next.getStackSize() > 0) {
            ...
            IMEMonitor<IAEFluidStack> itemInventory = bus.getProxy()
                .getStorage()
                .getFluidInventory();
            itr = itemInventory.getAvailableItems(
                AEApi.instance()
            ...
            if (check != null) {
                IAEFluidStack notadded = bus.getProxy()
                    .getStorage()
                    .getFluidInventory()
                    .injectItems(AEFluidStack.create(check), Actionable.SIMULATE, getActionSourceFor(bus));
                if (notadded != null && notadded.getStackSize() > 0) return TransferCheckResult.ofFail(
                    "inject.failure." + (bus.getProxy()
                        .isPowered()
                        && bus.getProxy()
    ...
    static BaseActionSource getActionSourceFor(Object o) {
        ...
        try {
            if (o instanceof MTEHatchOutputBusME) return (BaseActionSource) cache[0].invoke(o);
            if (o instanceof MTEHatchOutputME) return (BaseActionSource) cache[1].invoke(o);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private boolean moveToOutpus(IDualInputInventory opt, boolean checkSpace) {
        ...
        if (checkSpace) {
            for (int index = 0; index < i.length; ++index) {
                    ...
                    if (i[index] != null) {
                        ...
                        try {
                            ...
                            if (notadded != null && notadded.getStackSize() > 0) {
                                TransferCheckResult ret = TransferCheckResult.ofFail(
                                    "inject.failure." + (bus.getProxy()
                                        .isPowered()
                                        && bus.getProxy()
                                            .isActive())
                ...
                for (int x = 0; x < bus.getSizeInventory(); x++) {
                    if (bus.isValidSlot(x)) acc += space(i[index], bus.getStackInSlot(x));
                }
        ...
        for (int index = 0; index < i.length; ++index) {
            if (mOutputBusses.get(index) instanceof MTEHatchOutputBusME) {
                ...
                try {
                    IAEItemStack notadd = ((MTEHatchOutputBusME) mOutputBusses.get(index)).getProxy()
                        .getStorage()
                        .getItemInventory()
                        .injectItems(
                            AEApi.instance()
                            ...
                            Actionable.MODULATE,
                            getActionSourceFor(mOutputBusses.get(index)));
                    if (notadd != null) i[index].stackSize = (int) notadd.getStackSize();
                    else i[index].stackSize = 0;
                } catch (Exception e) {
        ...
        for (int index = 0; index < f.length; ++index) {
            if (mOutputHatches.get(index) instanceof MTEHatchOutputME) {
                ...
                try {
                    IAEFluidStack notadd = ((MTEHatchOutputME) mOutputHatches.get(index)).getProxy()
                        .getStorage()
                        .getFluidInventory()
                        .injectItems(
                            AEApi.instance()
                            ...
                            Actionable.MODULATE,
                            getActionSourceFor(mOutputHatches.get(index)));
                    if (notadd != null) f[index].amount = (int) notadd.getStackSize();
                    else f[index].amount = 0;
                } catch (Exception e) {
    ...
    public CheckRecipeResult checkProcessing() {
        ...
        calculateOverclockedNessMultiInternalX(120, 100, 1, inputVoltage, false);
    ...
    public void addUIWidgets(com.gtnewhorizons.modularui.api.screen.ModularWindow.Builder builder,
        ...
        builder.widget(createPowerSwitchButton(builder))
            ...
            .widget(new FakeSyncWidget.BooleanSyncer(() -> blocking, val -> { blocking = val; }).setSynced(true, true))
            .widget(
                new FakeSyncWidget.BooleanSyncer(() -> lockRecipe, val -> { lockRecipe = val; }).setSynced(true, false))
            .widget(
                new FakeSyncWidget.BooleanSyncer(() -> port != null, val -> { clientport = val; })
                    .setSynced(true, false))

            .widget(new FakeSyncWidget.IntegerSyncer(() -> count, val -> { count = val; }).setSynced(true, false))
            .widget(new FakeSyncWidget.IntegerSyncer(() -> cd, val -> { cd = val; }).setSynced(true, false))
            .widget(new FakeSyncWidget.IntegerSyncer(() -> cdmax, val -> { cdmax = val; }).setSynced(true, false))
            .widget(new FakeSyncWidget.BooleanSyncer(() -> yield, val -> { yield = val; }).setSynced(true, false))
            .widget(
                new FakeSyncWidget.IntegerSyncer(
                    () -> blockingNotAvailableReason,
                    val -> { blockingNotAvailableReason = val; }).setSynced(true, false));

    }

    ButtonWidget createBlockingModeButton(IWidgetBuilder<?> builder) {

        Widget button = new ButtonWidget().setOnClick((clickData, widget) -> { blocking = !blocking; })
        ...
        IntStream.range(0, Integer.valueOf(StatCollector.translateToLocal("proghatches.ingredientdistr.blocking.desc")))
            .forEach(
                s -> button.addTooltip(LangManager.translateToLocal("proghatches.ingredientdistr.blocking.desc." + s)));

        return (ButtonWidget) button;
    }

    public static boolean addHatch(IngredientDistributor thiz, IGregTechTileEntity aTileEntity, short s) {
        if (thiz.hasHatchThisLayer) {
            return false;
        }
    ...
    protected void drawTexts(DynamicPositionedColumn screenElements, SlotWidget inventorySlot) {
        ...
        screenElements.widget(new TextWidget().setStringSupplier(() -> lastfail == null ? "" : lastfail.format()

        )
            .setDefaultColor(COLOR_TEXT_WHITE.get())
            .setEnabled(widget -> { return (getBaseMetaTileEntity().isAllowedToWork()) && lastfail != null; }));
        ...
        screenElements.widget(TextWidget.dynamicString(() -> "lock:" + lockRecipe

        )
            .setDefaultColor(COLOR_TEXT_WHITE.get())
            .setEnabled(widget -> clientport));

        screenElements.widget(TextWidget.dynamicString(() -> "count:" + count

        )
            .setDefaultColor(COLOR_TEXT_WHITE.get())
            .setEnabled(widget -> clientport));

        screenElements.widget(TextWidget.dynamicString(() -> cd + "/" + cdmax
        ...
        screenElements.widget(
            new TextWidget().setStringSupplier(
                () -> StatCollector.translateToLocal(
                    ...
                    "proghatch.ingbuf.noblocking.reason." + blockingNotAvailableReason)
    ...
    protected void calculateOverclockedNessMultiInternalX(long aEUt, int aDuration, int mAmperage, long maxInputVoltage,
        boolean perfectOC) {
        ...
        OverclockCalculator calculator = (new OverclockCalculator()).setRecipeEUt(aEUt)
            .setEUt(GTValues.V[tier] * (long) mAmperage)
    ...
    // ===================== MUI2 =====================
    // Ported from the MUI1 addUIWidgets/drawTexts above (now dead code kept for reference):
    // status/debug text lines, blocking-mode toggle, and the synced TransferCheckResult.
    static final TransferCheckResult EMPTY_FAIL = TransferCheckResult.ofSuccess();
    ...
    private static class Gui extends gregtech.common.gui.modularui.multiblock.base.MTEMultiBlockBaseGui<IngredientDistributor> {

        Gui(IngredientDistributor mb) {
            super(mb);
        }

        @Override
        protected void registerSyncValues(com.cleanroommc.modularui.value.sync.PanelSyncManager syncManager) {
            super.registerSyncValues(syncManager);
            syncManager.syncValue(
                "ingd_lockRecipe",
                new com.cleanroommc.modularui.value.sync.BooleanSyncValue(
                    () -> multiblock.lockRecipe, v -> multiblock.lockRecipe = v));
            syncManager.syncValue(
                "ingd_port",
                new com.cleanroommc.modularui.value.sync.BooleanSyncValue(
                    () -> multiblock.port != null, v -> multiblock.clientport = v));
            syncManager.syncValue(
                "ingd_count",
                new com.cleanroommc.modularui.value.sync.IntSyncValue(
                    () -> multiblock.count, v -> multiblock.count = v));
            syncManager.syncValue(
                "ingd_cd",
                new com.cleanroommc.modularui.value.sync.IntSyncValue(() -> multiblock.cd, v -> multiblock.cd = v));
            syncManager.syncValue(
                "ingd_cdmax",
                new com.cleanroommc.modularui.value.sync.IntSyncValue(
                    () -> multiblock.cdmax, v -> multiblock.cdmax = v));
            syncManager.syncValue(
                "ingd_yield",
                new com.cleanroommc.modularui.value.sync.BooleanSyncValue(
                    () -> multiblock.yield, v -> multiblock.yield = v));
            syncManager.syncValue(
                "ingd_noblockreason",
                new com.cleanroommc.modularui.value.sync.IntSyncValue(
                    () -> multiblock.blockingNotAvailableReason, v -> multiblock.blockingNotAvailableReason = v));
            // reuse TransferCheckResult's existing PacketBuffer ser/deser; identity compare matches the
            // MUI1 FakeSyncWidget behaviour (lastfail is replaced, not mutated, on change)
            syncManager.syncValue(
                "ingd_lastfail",
                com.cleanroommc.modularui.value.sync.GenericSyncValue.builder(TransferCheckResult.class)
                    .getter(() -> multiblock.lastfail == null ? EMPTY_FAIL : multiblock.lastfail)
                    .setter(v -> multiblock.lastfail = (v == null || v.isSuccess) ? null : v)
                    .adapter(
                        new com.cleanroommc.modularui.utils.serialization.IByteBufAdapter<TransferCheckResult>() {

                            @Override
                            public TransferCheckResult deserialize(net.minecraft.network.PacketBuffer buffer) {
                                return TransferCheckResult.deser(buffer);
                            }

                            @Override
                            public void serialize(net.minecraft.network.PacketBuffer buffer,
                                TransferCheckResult t) {
                                TransferCheckResult.ser(buffer, t);
                            }

                            @Override
                            public boolean areEqual(TransferCheckResult t1, TransferCheckResult t2) {
                                return t1 == t2;
                            }
                        })
                    .build());
            ...
            return super.createTerminalTextWidget(syncManager, parent)
                .child(
                    com.cleanroommc.modularui.api.drawable.IKey
                        .dynamic(() -> multiblock.lastfail == null ? "" : multiblock.lastfail.format())
                        .color(com.cleanroommc.modularui.utils.Color.WHITE.main)
                        .asWidget()
                        .setEnabledIf(
                            w -> multiblock.getBaseMetaTileEntity().isAllowedToWork() && multiblock.lastfail != null)
                        .marginBottom(2)
                        .fullWidth())
                .child(com.cleanroommc.modularui.api.drawable.IKey.dynamic(() -> {
                    if (multiblock.yield) return StatCollector.translateToLocal("proghatch.ingbuf.yield");
                    if (multiblock.count > 0) return StatCollector.translateToLocal("proghatch.ingbuf.acquring");
                    if (multiblock.lockRecipe) return StatCollector.translateToLocal("proghatch.ingbuf.locked");
                    return StatCollector.translateToLocal("proghatch.ingbuf.idle");
                })
                    .color(com.cleanroommc.modularui.utils.Color.WHITE.main)
                    .asWidget()
                    .setEnabledIf(w -> multiblock.clientport)
                    .marginBottom(2)
                    .fullWidth())
                .child(
                    com.cleanroommc.modularui.api.drawable.IKey.dynamic(() -> "lock:" + multiblock.lockRecipe)
                        .color(com.cleanroommc.modularui.utils.Color.WHITE.main)
                        .asWidget()
                        .setEnabledIf(w -> multiblock.clientport)
                        .marginBottom(2)
                        .fullWidth())
                .child(
                    com.cleanroommc.modularui.api.drawable.IKey.dynamic(() -> "count:" + multiblock.count)
                        .color(com.cleanroommc.modularui.utils.Color.WHITE.main)
                        .asWidget()
                        .setEnabledIf(w -> multiblock.clientport)
                        .marginBottom(2)
                        .fullWidth())
                .child(
                    com.cleanroommc.modularui.api.drawable.IKey.dynamic(() -> multiblock.cd + "/" + multiblock.cdmax)
                        .color(com.cleanroommc.modularui.utils.Color.WHITE.main)
                        .asWidget()
                        .setEnabledIf(w -> multiblock.clientport)
                        .marginBottom(2)
                ...
                .child(
                    com.cleanroommc.modularui.api.drawable.IKey
                        .dynamic(
                            () -> StatCollector.translateToLocal(
                                "proghatch.ingbuf.noblocking.reason." + multiblock.blockingNotAvailableReason))
                        .color(com.cleanroommc.modularui.utils.Color.RED.main)
                        .asWidget()
                        .setEnabledIf(w -> multiblock.blockingNotAvailableReason > 0)
                        .marginBottom(2)
                        .fullWidth());
        }
                ...
                .child(new com.cleanroommc.modularui.widgets.CycleButtonWidget().stateCount(2)
                    .value(
                        (com.cleanroommc.modularui.value.sync.IntSyncValue) new com.cleanroommc.modularui.value.sync.IntSyncValue(
                            () -> multiblock.blocking ? 1 : 0,
                            v -> multiblock.blocking = v == 1).allowC2S())
                    ...
                    .stateOverlay(1, gregtech.api.modularui2.GTGuiTextures.OVERLAY_BUTTON_POWER_SWITCH_ON)
                    .stateOverlay(0, gregtech.api.modularui2.GTGuiTextures.OVERLAY_BUTTON_POWER_SWITCH_OFF)
                    .size(18, 18)
                    .marginRight(2)
                    .tooltipBuilder(t -> {
                        t.addLine(
                            com.cleanroommc.modularui.api.drawable.IKey
                                .str(StatCollector.translateToLocal("proghatches.ingredientdistr.blocking")));
                        int n = Integer
                            .valueOf(StatCollector.translateToLocal("proghatches.ingredientdistr.blocking.desc"));
                        for (int i = 0; i < n; i++) t.addLine(
                            com.cleanroommc.modularui.api.drawable.IKey.str(
                                LangManager.translateToLocal("proghatches.ingredientdistr.blocking.desc." + i)));
                    }));
        }
    }
