...
public class PatternDualInputHatchInventoryMappingSlave<T extends DualInputHatch & IDualInputHatch & IMetaTileEntity> extends MTETieredMachineBlock implements IAddUIWidgets, ICraftingMedium, ICustomNameObject, IGridProxyable, IInterfaceViewable, IPowerChannelState, IActionHost, ICraftingProvider, IMultiplePatternPushable, IDataCopyablePlaceHolder {
    ...
    public PatternDualInputHatchInventoryMappingSlave(String aName, int aTier, int aInvSlotCount, String[] aDescription, ITexture[][][] aTextures) {
    ...
    public void saveNBTData(NBTTagCompound aNBT) {
        ...
        for (int i = 0; i < pattern.length; i++) {
            final int ii = i;
            Optional.ofNullable(pattern[i]).map(s -> s.writeToNBT(new NBTTagCompound())).ifPresent(s -> tag.setTag("i" + ii, s));
        }
    ...
    public void loadNBTData(NBTTagCompound aNBT) {
        if (!aNBT.hasKey("x")) return;
        additionalConnection = aNBT.getBoolean("additionalConnection");
        NBTTagCompound tag = aNBT.getCompoundTag("patternSlots");
        if (tag != null) for (int i = 0; i < pattern.length; i++) {
            pattern[i] = Optional.ofNullable(tag.getCompoundTag("i" + i)).map(ItemStack::loadItemStackFromNBT).orElse(null);
        }
    ...
    public ITexture[] getTexture(IGregTechTileEntity baseMetaTileEntity, ForgeDirection side, ForgeDirection facing, int colorIndex, boolean active, boolean redstoneLevel) {
        ...
        if (side != facing)
            ...
            return new ITexture[]{Textures.BlockIcons.MACHINE_CASINGS[mTier][colorIndex + 1], TextureFactory.of(MyMod.iohub, BlockIOHub.magicNO_inv_me_slave)};
    ...
    private boolean tryLinkDataStick(EntityPlayer aPlayer) {
        ...
        if (!dataStick.hasTagCompound() || !dataStick.stackTagCompound.getString("type").equals("ProgHatchesDualInput")) {
            return false;
        }
    ...
    public boolean onRightclick(IGregTechTileEntity aBaseMetaTileEntity, EntityPlayer aPlayer, ForgeDirection side, float aX, float aY, float aZ) {
        ...
        if (is != null && is.getItem() instanceof ToolQuartzCuttingKnife) {
            ...
            aPlayer.openGui(AppEng.instance(), GuiBridge.GUI_RENAMER.ordinal() << 5 | (side.ordinal()), te.getWorld(), te.getXCoord(), te.getYCoord(), te.getZCoord());
            return true;
        }
        ...
        if (getMaster() != null) {
            if (!aBaseMetaTileEntity.isClientSide()) {
                ...
                MyMod.net.sendTo(new MasterSetMessage(aBaseMetaTileEntity.getXCoord(), aBaseMetaTileEntity.getYCoord(), aBaseMetaTileEntity.getZCoord(), this), (EntityPlayerMP) aPlayer);
    ...
    public ForgeDirection getMasterFront() {
        if (master == null) return this.getBaseMetaTileEntity().getFrontFacing();// throw new
        // RuntimeExcpetion()?
    ...
    public void getWailaBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor, IWailaConfigHandler config) {
        ...
        if (tag.hasKey("masterX")) {
            currenttip.add("Bound to " + tag.getInteger("masterX") + ", " + tag.getInteger("masterY") + ", " + tag.getInteger("masterZ"));
        }
    ...
    public void addUIWidgets(Builder builder, UIBuildContext buildContext) {
        ...
        if (getMaster() instanceof IAddUIWidgets) {
            ...
            builder.widget(new ButtonWidget().setOnClick((clickData, widget) -> {
                    if (!widget.getContext().isClient()) widget.getContext().openSyncedWindow(989898);
                }).setPlayClickSound(true).setBackground(GTUITextures.BUTTON_STANDARD, GTUITextures.OVERLAY_BUTTON_PLUS_LARGE).addTooltips(ImmutableList.of(LangManager.translateToLocalFormatted("programmable_hatches.gt.pattern.mapping"))).setSize(16, 16)
    ...
    public AENetworkProxy getProxy() {
        if (gridProxy == null) {
            gridProxy = new AENetworkProxy(this, "proxy", new ItemStack(GregTechAPI.sBlockMachines, 1, this.getBaseMetaTileEntity().getMetaTileID()), true);
    ...
    public void provideCrafting(ICraftingProviderHelper craftingTracker) {
        ...
        for (int index = 0; index < pattern.length; index++) {
            ...
            try {
                details = ((ICraftingPatternItem) slot.getItem()).getPatternForItem(slot, this.getBaseMetaTileEntity().getWorld());
            } catch (Exception e) {
            }
            if (details == null) {
                GTMod.GT_FML_LOGGER.warn("Found an invalid pattern at " + getBaseMetaTileEntity().getCoords() + " in dim " + getBaseMetaTileEntity().getWorld().provider.dimensionId);
                continue;
            }
    ...
    protected ModularWindow createPatternWindow(final EntityPlayer player) {
        ...
        builder.setPos((a, b) -> new Pos2d(PARENT_WIDTH + b.getPos().getX(), 0 + b.getPos().getY()));
        ...
        for (int i = 0; i < 36; i++) {
            ...
            }.setShiftClickPriority(-1).setFilter(itemStack -> itemStack.getItem() instanceof ICraftingPatternItem).setChangeListener(() -> {
                onPatternChange();
            }).setPos((i % 4) * 18 + 3, (i / 4) * 18 + 3).setBackground(getGUITextureSet().getItemSlot(), GTUITextures.OVERLAY_SLOT_PATTERN_ME));
    ...
    public int pushPatternMulti(ICraftingPatternDetails patternDetails, InventoryCrafting table, int maxTodo) {
        if (!reobf.proghatches.main.Config.fastPatternDualInput) return 0;
    ...
    public void onBlockDestroyed() {
        ...
        for (int i = 0; i < pattern.length; i++) {
            ...
            if ((tItem != null) && (tItem.stackSize > 0)) {
                final EntityItem tItemEntity = new EntityItem(aWorld, aX + XSTR_INSTANCE.nextFloat() * 0.8F + 0.1F, aY + XSTR_INSTANCE.nextFloat() * 0.8F + 0.1F, aZ + XSTR_INSTANCE.nextFloat() * 0.8F + 0.1F, new ItemStack(tItem.getItem(), tItem.stackSize, tItem.getItemDamage()));
